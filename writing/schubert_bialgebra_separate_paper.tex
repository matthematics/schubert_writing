...
\textbf{[Identity \eqref{eq:clpperm} for $\maxd(\wof Y)=m$, $p\geq 2$ remains to be written. It has been verified by computer for all bounded RC graphs of height at most $5$ and length at most $5$. Two reductions: (a) it suffices to treat $Y$ with rows $p+2,\ldots,m$ empty, since $D^{p}\circ D^{p+1}=D^{p}$ at every level implies $D^p\circ D^{n}=D^p$ by induction on $n-p$; (b) by Proposition~\ref{proposition:zerocrystal}, $\zeromap$ is a composite of Little bumps at roots $(m',m'+1)$ with $m'>p$, and \eqref{eq:clpzero} is equivalent to the statement that $D^p(T)$ is unchanged by any Little bump of $T\in\RC_{\leq p}$ at an inversion $(a,b)$ with $a>p$ (verified for length $\leq 7$), i.e.\ to the confluence of the rewriting system on $\RC_{\leq p}$ generated by such bumps. Note that the one-step statement ``erasing rows $p+1,\ldots,m$ commutes with $\zeromap$'' is false, and $\wof{D^p(Y)}$ is not a function of $\wof{Y_{\leq p}}$ and $\wtt(Y)$ alone; only the fully iterated identity holds.]}
\end{proof}
