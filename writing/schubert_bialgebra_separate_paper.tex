...
and multiplicatively the product of two basis elements indexed by $a\in \mathrm{Comp}_p$ and $b\in \mathrm{Comp}_q$ is the basis element indexed by the concatenation of $a$ and $b$, which is an element of $\mathrm{Comp}_{p+q}$. We will denote this product by $\sqcupplus$, writing $a\sqcupplus b$ for the concatenation. The coproduct on $\dcoma$ is given by splitting sums of exponents, that is to say
$$\Delta(c) = \sum_{a+b=c} a\otimes b$$
where $a$ and $b$ are weak compositions of the same length as $c$ and $a+b$ denotes the pointwise sum. The counit is given by $\varepsilon(c)=0$ unless $c$ is the empty composition, in which case $\varepsilon(c)=1$. With these definitions, $\coma$ and $\dcoma$ are dual bialgebras over $\mathbb{Z}$ (with $\coma$ non-unital: the constant $1\in\coma_n$ is a unit for $\coma_n$ only), a fact that is straightforward to verify. Neither $\coma$ nor $\dcoma$ is connected, and neither is a Hopf algebra. A ``Littlewood--Richardson rule'' for a basis of the dual algebra is equivalent to a branching/splitting formula for the corresponding polynomial basis under the partition of variables into two blocks $\{x_1,\ldots,x_p\}$ and $\{x_{p+1},\ldots,x_{p+q}\}$.
...
For the reader familiar with bumpless pipe dreams \cite{lam2021back}, our transition map for RC graphs is realized trivially for bumpless pipe dreams. Specifically, while (reduced) bumpless pipe dreams are typically defined in such a way that they cannot deviate from a square shape, there is indeed a consistent way to remove rows from a bumpless pipe dream unambiguously. This was observed by Yu in \cite[Lemma~3.18]{Yu2025EmbeddingBP} in the definition of a $k$-row BPD. Weigandt in \cite{Weigandt2021} defines a transition formula $\mathbf{t}:\mathrm{BPD}_n\to\mathrm{BPD}_{n-1}$, and while the image is realized as square, it agrees with Yu's notion of an $n-1$-row BPD. Under the Gao--Huang bijection \cite{GaoHuang2023} between RC graphs and bumpless pipe dreams, $\zeromap$ is the deletion of the last row (Proposition~\ref{proposition:zerotrunc}); this is how the properties of $\zeromap$ and the associativity of our product are proved in Section~\ref{section:bpd}.

This simplicity does not come for free, however; the $\trm^p$ operation on BPDs is no longer trivial.

We conjecture a far stronger result that would result in a generalization of the Schubert bialgebra to $K$-theory. Let $\mathrm{MBPD}(n)$ denote the set of $n$-row marked bumpless pipe dreams and let $\mathrm{Pipe}(n)$ denote the set of $n$-row not-necessarily-reduced pipe dreams. Huang, Shimozono, and Yu in \cite{HuangShimozonoYu2024} present a bijection $\Phi:\mathrm{MBPD}(n)\to\mathrm{Pipe}(n)$.

For $P\in\mathrm{Pipe}(n)$ and $p\leq n$, let 
$$\clp^p(P) = \Phi(\mathbf{t}^{n-p}(\Phi^{-1}(P))).$$
Define a function $\sqcupplus:\mathrm{Pipe}(m)\times\mathrm{Pipe}(n)\to\mathbb{Z}^{\mathrm{Pipe}(m+n)}$ by
$$P\sqcupplus Q = \sum_{R\in \mathrm{Pipe}(m+n), \clp^m(R) = P, \trm^m(R) = Q} R.$$
Then
\begin{conjecture}
Define
$$\mathrm{BPipe} = \bigoplus_{n=0}^\infty \mathbb{Z}^{\mathrm{Pipe}(n)}.$$
Then we have the following:
\begin{itemize}
\item $\sqcupplus:\mathrm{Pipe}(n)\times\mathrm{Pipe}(m)\to\mathbb{Z}^{\mathrm{Pipe}(n+m)}$ extends to an associative product on $\mathrm{BPipe}$ for which it is a graded ring.
\item The map $\mathbf{f}:\mathrm{BPipe}\to\BRC$ such that
$$\mathbf{f}(P)=\begin{cases}
  P & \text{if } P\text{ is reduced} \\
  0 & \text{otherwise}
\end{cases}$$
induces a surjective homomorphism of rings 
$$\mathbf{f}:\left(\mathrm{BPipe}, \sqcupplus\right) \to \left(\BRC, \sqcupplus\right).$$

\item Let $G_w^n$ for $w\in S_\infty$ be the basis of $\dcoma$ dual to the basis of Grothendieck polynomials (with the $n$ representing the max number of variables). Then the map $\mathrm{BPipe}\to \dcoma$ defined on the $n$-row pipe dream $P\in\mathrm{Pipe}(w,n)$ for $w\in S_\infty$ by
$$P\mapsto G_w^n$$
extends to a surjective homomorphism of rings.
\end{itemize}
\end{conjecture}

In effect this would generalize Little's bumping algorithm to the $0$-Hecke words of permutations that come from pipe dreams representing the monomials of Grothendieck polynomials, yielding word-level combinatorial transition algorithm.
...
\begin{definition}[Two Pieri relations $\tom{k}$ and $\Tom{k}$]\label{definition:pieri}
...
\begin{definition}[Divided differences and skew divided differences]\label{definition:divdiff}
For $i\geq 1$ let $s_i$ act on polynomials in $x$ by exchanging $x_i$ and $x_{i+1}$, and define the divided difference operator
$$\partial^i = \frac{1}{x_i-x_{i+1}}(1-s_i)\text{.}$$
The $\partial^i$ satisfy the nil-Coxeter relations, so for $w\in S_\infty$ with reduced word $w=s_{i_1}\cdots s_{i_k}$ the operator $\partial^w=\partial^{i_1}\cdots\partial^{i_k}$ is well defined. We write $\delta^i$ and $\delta^w$ for the same operators acting on the $y$ variables of a double Schubert polynomial, so $\delta^i=\frac{1}{y_i-y_{i+1}}(1-s_i)$ with $s_i$ now exchanging $y_i$ and $y_{i+1}$. On double Schubert polynomials,
$$\partial^i\sch_w(x;y)=\begin{cases}\sch_{ws_i}(x;y)&\text{if }\ell(ws_i)<\ell(w),\\ 0&\text{otherwise,}\end{cases}\qquad\qquad \delta^i\sch_w(x;y)=\begin{cases}-\sch_{s_iw}(x;y)&\text{if }\ell(s_iw)<\ell(w),\\ 0&\text{otherwise,}\end{cases}$$
so that $\delta^v\sch_w(x;y)=(-1)^{\ell(v)}\sch_{vw}(x;y)$ whenever $\ell(vw)=\ell(w)-\ell(v)$. Since each $\delta^i$ changes sign under the substitution $y\mapsto -y$, the signs cancel on $\sch_w(x;-y)$:
$$\delta^v\bigl[\sch_w(x;-y)\bigr]=\sch_{vw}(x;-y)\qquad\text{whenever }\ell(vw)=\ell(w)-\ell(v)\text{.}$$

The \emph{skew divided difference operators} $\delta_u^w$ for $u\leq w$ in Bruhat order were defined originally in \cite{notes}; we use the conventions of \cite{samuelleibniz}, in which they are characterized by the Leibniz formula
$$\delta^w(pq) = \sum_{u\leq w} \delta^u(p)\,\delta_u^w(q)$$
for arbitrary polynomials $p$ and $q$, together with its iterated form $\delta_u^w(pq)=\sum_{u\leq v\leq w}\delta_u^v(p)\,\delta_v^w(q)$. The operators $\partial_u^w$ on the $x$ variables are defined identically.
\end{definition}
...
We define a commutative algebra $\coma$ over the integers as follows. For each $n\geq 0$, define $\coma_n$ to be the polynomial ring over $\mathbb{Z}$ in the variables $x_1,\ldots,x_n$ (so $\coma_0=\mathbb{Z}$). Then define
$$\coma =\bigoplus_{n=0}^\infty \coma_n$$
The multiplication within each $\coma_n$ is the usual multiplication of polynomials, and if $a\in \coma_m$ and $b\in \coma_n$ with $m\neq n$, then
$$ab = 0\text{.}$$
That is, $\coma$ is the direct sum of the rings $\coma_n$, including $\coma_0$; it is a commutative ring without unit, and the constant polynomial $1\in\coma_n$ is a unit for the summand $\coma_n$ only. The $\mathbb{Z}$-module structure is the evident one.

\begin{remark}[Fat identities]\label{remark:fatidentity}
The constant polynomial $1\in\coma_n$ acts as a unit on $\coma_n$ itself but is \emph{not} a unit of $\coma$ as a whole: by the rule above it annihilates every $\coma_m$ with $m\neq n$. We will refer to these length-$n$ units informally as ``fat identities''; $\coma$ itself has no unit. This is the conceptual hinge that makes $\coma$ a bialgebra rather than a Hopf algebra. The triviality of cross-length products is what allows the deconcatenation coproduct $\Delta$ defined below to be a homomorphism of rings (if $1\in\coma_0$ acted as a global unit, $\Delta$ would fail to be multiplicative already on $x_1\cdot x_1x_2=0$); conversely, it forces $\coma$ to fail to be connected, so $\coma$ admits no antipode.
\end{remark}
...
We define a coproduct $\Delta:\coma\to\coma\otimes\coma$ on the basis element $x_c$ by
...
\end{lemma}
\begin{proof}
Let $a,b$ be weak compositions of lengths $m$ and $n$, and for a weak composition $c$ of length $r$ and $0\leq k\leq r$ write $c_{\leq k}$ and $c_{>k}$ for the first $k$ and the remaining $r-k$ entries of $c$. Then
$$\Delta(x_a)\Delta(x_b)=\sum_{j=0}^{m}\sum_{k=0}^{n}x_{a_{\leq j}}x_{b_{\leq k}}\otimes x_{a_{>j}}x_{b_{>k}}\text{.}$$
The $(j,k)$ term vanishes unless $j=k$ and $m-j=n-k$, that is, unless $m=n$ and $j=k$. Hence $\Delta(x_a)\Delta(x_b)=0=\Delta(x_ax_b)$ when $m\neq n$, while for $m=n$
$$\Delta(x_a)\Delta(x_b)=\sum_{k=0}^n x_{a_{\leq k}+b_{\leq k}}\otimes x_{a_{>k}+b_{>k}}=\sum_{k=0}^n x_{(a+b)_{\leq k}}\otimes x_{(a+b)_{>k}}=\Delta(x_{a+b})=\Delta(x_ax_b)\text{.}$$
\end{proof}
...
\subsection{The elementary basis}\label{section:elementary}
...
For the reverse transition, we need a lemma on the skew divided difference operators $\delta_u^w$ of Definition~\ref{definition:divdiff}. Recall that $\delta^i=\frac{1}{y_i-y_{i+1}}(1-s_i)$ acts on the $y$ variables, that $\delta^w=\delta^{i_1}\cdots\delta^{i_k}$ for a reduced word $w=s_{i_1}\cdots s_{i_k}$, and that the skew operators are characterized by the Leibniz formula $\delta^w(pq)=\sum_{u\leq w}\delta^u(p)\,\delta_u^w(q)$. The lemma computes the application of a skew divided difference operator of degree $-n$ to a polynomial of degree $n$, which yields an integer for integer-coefficient polynomials. Specifically, it is a formula for application of such an operator with $\ell(u,w)=n$ to $y_p^n$ for some $p>0$.
...
$$\delta_u^w(y_p^n) = \sum_{C\in \achain{p}(u,w)} (-1)^{\sigma(C)}.$$
\end{lemma}
\begin{proof}

  

Consider the case where $\ell(u,w)=1$. Then there is a transposition $t_{ij}$ such that $w = ut_{ij}$. Assume that $i<j$. If $p\notin\{i,j\}$, then $\delta_u^w(y_p)=0$. If $p=i$, then $\delta_u^w(y_p) = \delta^{ij}(y_p) = 1$. If instead $j=p$, then $\delta_u^w(y_p) = \delta^{ij}(y_p) = -1$. In all cases, the result holds.

Now, consider the case where $\ell(u,w)=n>1$. We have that
$$y_p^n = y_p\cdot y_p^{n-1}$$
Applying the Leibniz formula, we have
$$\delta_u^w(y_p^n) = \sum_{u\prec u'}\delta_u^{u'}(y_p)\delta_{u'}^w(y_p^{n-1})$$
By the above, $\delta_u^{u'}(y_p)$ is nonzero if and only if $u\prec u'$ is a cover in the Bruhat order such that $u(p)\neq u'(p)$. In this case, $\delta_u^{u'}(y_p)$ is either $1$ or $-1$ depending on the number of indices $j<p$ such that $u(j)\neq u'(j)$, which is either $0$ or $1$, respectively. By induction, 
$$\delta_{u'}^w(y_p^{n-1}) = \sum_{C'\in \achain{p}(u',w)} (-1)^{\sigma(C')}.$$
Hence, 
$$\delta_u^{u'}(y_p)\delta_{u'}^w(y_p^{n-1})=\begin{cases}
...
\end{proof}
Using a minor sleight of hand, this affords us an explicit formula for transition of a Schubert polynomial into the elementary basis. We first name the combinatorial data that will index the terms of the formula.

\begin{definition}
For $k,p>0$ let 
$$E_k(x;y_p)=\prod_{i=1}^k (x_i - y_p)\text{,}$$
...
\begin{lemma} \label{lemma:doublediff}
  ...
  $$\delta_u^w(E_k(x;-y_p))=\sum_{C\in\achain{p}(u,w)}(-1)^{\sigma(C)}e_{k-\ell(u,w)}^k+\mathcal{O}(y_p)$$
\end{lemma}
...
\begin{definition}\label{definition:elemcoeff}
Fix $w\in S_\infty$ with $\maxd(w)\leq n$. Let
$$\lambda = (n, n, \ldots, n, n - 1,\ldots, 1)$$
be the partition with parts $n-1,n-2,\ldots,1$ preceded by some number of parts equal to $n$, chosen large enough that $\ell(ww_\lambda^{-1})=|\lambda|-\ell(w)$, where $w_\lambda$ is the dominant permutation with $\code^*(w_\lambda)=\lambda$. Write $N=\ell(\lambda)$.

A \emph{path} for $w$ is a sequence $P=(C_1,\ldots,C_N)$ of chains $C_i\in\achain{i}(u_{i-1},u_i)$ for some sequence of permutations $1=u_0\leq u_1\leq\cdots\leq u_N=ww_\lambda^{-1}$; denote the set of such paths by $\mathrm{Path}_\lambda(w)$. For $P\in\mathrm{Path}_\lambda(w)$ set
$$\sigma(P) = \sum_{i=1}^{N} \sigma(C_i),\qquad \alpha(P) = \bigl(\lambda_{N + 1 - i}-|C_{N + 1 - i}|\bigr)_{i=1}^{N},$$
so that $\alpha(P)$ is an $n$-elementary weak composition. Finally, for each weak composition $\alpha$ define
$$E^{\alpha;n}_w = \sum_{\substack{P \in \mathrm{Path}_\lambda(w)\\ \alpha(P)=\alpha}}(-1)^{\sigma(P)}.$$
\end{definition}

\begin{theorem}[Transition to the elementary basis] \label{theorem:elemtrans}
	For each $w$ with $\maxd(w)\leq n$, we have
	$$\sch_w^n = \sum_{\alpha}E_w^{\alpha;n}e_{\alpha}^n.$$
In particular, since $\mathcal{E}_n$ is a basis of $\coma_n$, the integers $E_w^{\alpha;n}$ do not depend on the number of parts equal to $n$ in $\lambda$ (identifying $\alpha$ with its extensions by trailing zeros).
\end{theorem}
\begin{proof}
Let $\lambda$, $w_\lambda$, and $N$ be as in Definition~\ref{definition:elemcoeff}. By \cite[(6.14)]{notes}, the double Schubert polynomial of the dominant permutation $w_\lambda$ factors as
$$\sch_{w_{\lambda}}(x;-y)=\prod_{i=1}^N E_{\lambda_i}(x;-y_i).$$
Recall from Definition~\ref{definition:divdiff} that $\delta^v$ acts on the $y$ variables with $\delta^v\sch_w(x;y)=(-1)^{\ell(v)}\sch_{vw}(x;y)$ whenever $\ell(vw)=\ell(w)-\ell(v)$, and that the sign disappears on $\sch_w(x;-y)$ because each $\delta^i$ is odd under $y\mapsto -y$: $\delta^v[\sch_w(x;-y)]=\sch_{vw}(x;-y)$. By the choice of $\lambda$, this gives
$$\sch_w(x;-y) = \delta^{ww_\lambda^{-1}}\prod_{i=1}^N E_{\lambda_i}(x;-y_i),$$
with $\delta^{ww_\lambda^{-1}}$ acting on the variables $y_i$ appearing in the product.
Expand the right-hand side using the Leibniz formula iterated across the $N$ factors: the result is a sum over sequences $1=u_0\leq u_1\leq\cdots\leq u_N=ww_\lambda^{-1}$ of the products $\prod_i\delta_{u_{i-1}}^{u_i}E_{\lambda_i}(x;-y_i)$. Setting $y_i=0$ for all $i$ and invoking Lemma~\ref{lemma:doublediff} for each factor, this becomes
$$\sch_w(x)=\sum_{P=(C_1,\ldots,C_N)}(-1)^{\sigma(C_1)+\cdots+\sigma(C_N)}e_{\lambda_{1}-|C_1|}^{\lambda_1}e_{\lambda_2-|C_2|}^{\lambda_2}\cdots e_{\lambda_{N}-|C_{N}|}^{\lambda_{N}},$$
the sum running over $P\in\mathrm{Path}_\lambda(w)$. The monomial in the $P$-th term is $e_{\alpha(P)}^n$, and collecting terms by $\alpha(P)$ gives the stated expansion with coefficients $E_w^{\alpha;n}$.
\end{proof}
...
\begin{example}
	...
	\begin{align*}
		\sch_w^n &= e^5_{(0,0,0,0,5,1,1)} - e^5_{(0,0,0,0,4,2,1)} + e^5_{(0,0,0,0,4,3)}- e^5_{(0,1,0,0,4,1,1)} +e^5_{(0,1,0,0,3,2,1)}\\
		&+e^5_{(0,1,0,0,5,1)} - e^5_{(0,1,0,0,3,3)}
	\end{align*}
\end{example}
...
We characterize it with an explicit formula, in terms of the chains $\achain{p}(u,w)$ and the statistic $\sigma$ of \S\ref{section:elementary}.

\begin{definition}\label{definition:dualpath}
Let $u\in S_\infty$ with $\maxd(u)\leq n$. A \emph{dual path} for $(u,n)$ is a sequence $P=(C_1,\ldots,C_n)$ of chains $C_i\in\achain{i}(u_{i-1},u_i)$ for some sequence of permutations $1=u_0\leq u_1\leq\cdots\leq u_n=u$; write $\mathrm{Path}^n(u)$ for the set of dual paths. For $P\in\mathrm{Path}^n(u)$ set
$$\sigma(P)=\sum_{i=1}^n\sigma(C_i),\qquad |P|=(|C_1|,\ldots,|C_n|)\text{,}$$
so that $|P|$ is a weak composition of length $n$ with $\bigl||P|\bigr|=\ell(u)$.
\end{definition}

\begin{theorem}\label{theorem:dualformula}
	For each permutation $u$ and integer $n$ with $\maxd(u)\leq n$,
	$$\dsch_u^{n} = \sum_{P\in\mathrm{Path}^n(u)} (-1)^{\sigma(P)}\,[\,|P|\,]\text{.}$$
	That is, the coefficient of $[\alpha]$ in $\dsch_u^n$ is the signed number of dual paths $P$ for $(u,n)$ with $|C_i|=\alpha_i$ for all $i$.
\end{theorem}
\begin{proof}
	By definition, the coefficient of $[\alpha]$ in $\dsch_u^n$ is the coefficient $c_{\alpha,u}$ of $\sch_u^n$ in the Schubert expansion of $x_\alpha$. Since $\partial^u\sch_v=\delta_{u,v}$ for $\ell(v)=\ell(u)$ and $\partial^u$ kills polynomials of degree less than $\ell(u)$ \cite{notes}, we have $c_{\alpha,u}=\partial^u(x_\alpha)$ when $|\alpha|=\ell(u)$, and $c_{\alpha,u}=0$ otherwise. Writing $x_\alpha=x_1^{\alpha_1}\cdots x_n^{\alpha_n}$ and applying the Leibniz formula of Definition~\ref{definition:divdiff} (in its $x$-variable form) iteratively across the $n$ factors,
	$$\partial^u(x_\alpha)=\sum_{1=u_0\leq u_1\leq\cdots\leq u_n=u}\ \prod_{i=1}^n\partial_{u_{i-1}}^{u_i}\bigl(x_i^{\alpha_i}\bigr)\text{.}$$
	Each factor $\partial_{u_{i-1}}^{u_i}(x_i^{\alpha_i})$ is homogeneous of degree $\alpha_i-\ell(u_{i-1},u_i)$ (zero if this is negative), and the degrees sum to $|\alpha|-\ell(u)=0$, so a term is a nonzero constant only if $\ell(u_{i-1},u_i)=\alpha_i$ for every $i$. For such terms Lemma~\ref{lemma:powerdiff}, which holds verbatim for $\partial_u^w(x_p^k)$, gives $\partial_{u_{i-1}}^{u_i}(x_i^{\alpha_i})=\sum_{C_i\in\achain{i}(u_{i-1},u_i)}(-1)^{\sigma(C_i)}$. Multiplying out yields the sum over dual paths $P$ with $|P|=\alpha$ of $(-1)^{\sigma(P)}$.
\end{proof}

\begin{remark}
Theorem~\ref{theorem:dualformula} can also be derived from the Cauchy formula $\sch_\mu(x;-y)=\sum_{v}\sch_v(x)\sch_{v\mu^{-1}}(y)$ for a dominant permutation $\mu$ with $\code(\mu)$ strictly decreasing and $\code_n(\mu)\geq\ell(u)$, $\code_{n+1}(\mu)=0$, by expanding $\sch_{u\mu^{-1}}(y)$ in the products $\prod_i e^{\code_i(\mu)}_{\code_i(\mu)-\alpha_i}(y)$ exactly as in the proof of Theorem~\ref{theorem:elemtrans}; the coefficients so obtained are independent of $\mu$.
\end{remark}
...
$$\dsch_w^3 = [022] - [013]$$
If we choose $[1,3,5,7,2,4,6]$ instead, we have
...
More generally, the product of $\dcoma$ is dual to splitting the variables of $\coma$ into two consecutive blocks. For permutations $w,u,v$ with $\maxd(w)\leq p+q$, $\maxd(u)\leq p$, $\maxd(v)\leq q$, define the \emph{branching coefficients} $b_{u,v}^w(p,q)$ by
\begin{equation}\label{eq:branching}
\sch_w(x_1,\ldots,x_{p+q})=\sum_{u,v}b_{u,v}^w(p,q)\,\sch_u(x_1,\ldots,x_p)\,\sch_v(x_{p+1},\ldots,x_{p+q})\text{;}
\end{equation}
these are well defined since the products $\sch_u(x_1,\ldots,x_p)\sch_v(x_{p+1},\ldots,x_{p+q})$ form a basis of $\mathbb{Z}[x_1,\ldots,x_{p+q}]$.

\begin{lemma}\label{lemma:dualbranch}
With notation as above,
$$\dsch_u^p\sqcupplus\dsch_v^q=\sum_{w}b_{u,v}^w(p,q)\,\dsch_w^{p+q}\text{.}$$
\end{lemma}
\begin{proof}
For $\alpha\in\dcoma_p$, $\beta\in\dcoma_q$ and $f\in\coma_{p+q}$ we have $\langle\alpha\sqcupplus\beta,f\rangle=\langle\alpha\otimes\beta,\Delta_{p,q}(f)\rangle$, where $\Delta_{p,q}:\coma_{p+q}\to\coma_p\otimes\coma_q$ is the component of $\Delta$; indeed both sides equal $\delta_{ab,c}$ on $\alpha=[a]$, $\beta=[b]$, $f=x_c$. Identifying $\coma_p\otimes\coma_q$ with polynomials in the two blocks of variables, $\Delta_{p,q}(f)=f(x_1,\ldots,x_{p+q})$ read as a polynomial in $(x_1,\ldots,x_p)$ and $(x_{p+1},\ldots,x_{p+q})$, so $\Delta_{p,q}(\sch_w^{p+q})=\sum_{u,v}b_{u,v}^w(p,q)\sch_u^p\otimes\sch_v^q$ by \eqref{eq:branching}. Hence the coefficient of $\dsch_w^{p+q}$ in $\dsch_u^p\sqcupplus\dsch_v^q$ is
\[\langle\dsch_u^p\sqcupplus\dsch_v^q,\sch_w^{p+q}\rangle=\langle\dsch_u^p\otimes\dsch_v^q,\Delta_{p,q}(\sch_w^{p+q})\rangle=b_{u,v}^w(p,q)\text{.}\qedhere\]
\end{proof}
...
which is also a surjective homomorphism of graded modules. In addition, there is the weight map $\omega:\BRC\to \dcoma$ defined by
$$\omega(R) = [\wtt(R)]$$
which is similarly a surjective homomorphism of graded modules.
...
\begin{corollary}\label{corollary:evschub}
...
\subsection{Zeroing out the last row}\label{section:bpd}

We proceed now to define a product on $\BRC$ turning it into a ring. To do this, we need a map $\zeromap:\BRC\to\BRC$ trimming empty rows from the bottom instead of from the top, which is far more delicate than trimming from the top. We define $\zeromap$ by Little's bumping algorithm \cite{little2003combinatorial} in its pipe dream form \cite{billey2019bijective}, and we prove its properties by transporting it along the Gao--Huang bijection \cite{GaoHuang2023} to bumpless pipe dreams, where deleting a row is literal. In this subsection an RC graph is a finite set $R\subseteq\mathbb P\times\mathbb P$ with $\ell(\wof R)=|R|$ (a reduced pipe dream); the height enters only where stated. We write $R_{\leq p}=\{(i,j)\in R:i\leq p\}$.
...
\end{tikzpicture}
\caption*{(a) The RC graph $R$; the crossing $(2,2)$ has right root $(3,4)$}
\end{minipage}
\hfill
% Fifth diagram
\begin{minipage}{0.45\textwidth}
...
\end{tikzpicture}
\caption*{(b) $\zeromap(R)$, of height $2$}
\end{minipage}

\caption{The map $\zeromap$ on $R=\{(1,2),(1,3),(2,2)\}$, of height $3$ and permutation $1432$. The crossing $(2,2)$ moves to $(2,1)$; the resulting word $3\,2\,2$ is not reduced, so $(1,2)$ moves to $(1,1)$, giving the reduced word $3\,1\,2$ of $1432\,t_{34}t_{13}=2413$. The empty row $3$ is then deleted.}
\label{fig:zeroing}
\end{figure}
...
\subsubsection*{Bumpless pipe dreams}
Fix $N$ larger than every row and column index that occurs below. A \emph{bumpless pipe dream} \cite{lam2021back} is a tiling of the $N\times N$ grid by six tiles: the blank tile, the crossing tile, the horizontal and vertical tiles, and two elbow tiles, the \emph{$r$-tile}, in which a pipe enters from the south and leaves to the east, and the \emph{$j$-tile}, in which a pipe enters from the west and leaves to the north, such that the resulting pipes enter the grid from the south edge, one in each column, travel north and east, leave through the east edge, one in each row, and no two pipes cross twice. The pipe entering at the bottom of column $j$ is \emph{pipe $j$}. The permutation $w$ of $B$ is defined by: the pipe leaving through row $i$ is pipe $w(i)$. We write $\mathrm{BPD}(w)$ for the set of bumpless pipe dreams of $w$, $\mathrm{blank}(B)$ for the set of blank tiles of $B$ and $B_{\leq p}$ for rows $1,\ldots,p$ of $B$. By \cite[Theorem~5.13]{lam2021back}, $\sch_w=\sum_{B\in\mathrm{BPD}(w)}\prod_{(i,j)\in\mathrm{blank}(B)}x_i$. Enlarging $N$ appends pipes that travel straight north and turn once, and appends horizontal tiles to the rows already present, so statements about $B_{\leq p}$ do not depend on $N$. Since a pipe travels north and east, the pipe leaving through row $i$ lies entirely in rows $\geq i$; we use this repeatedly.

Gao and Huang \cite{GaoHuang2023} construct, for every $w$, a bijection between $\mathrm{BPD}(w)$ and the RC graphs with permutation $w$ that sends $\mathrm{blank}(B)$ row by row to the crossings, i.e.\ row $i$ of $B$ has as many blank tiles as row $i$ of the RC graph has crossings \cite[Theorem~3.6]{GaoHuang2023}. We write $\Phi(R)\in\mathrm{BPD}(\wof R)$ for the bumpless pipe dream corresponding to the RC graph $R$; $\Phi$ is injective and depends only on the set $R$.

We recall Little's bumping operation on reduced words \cite{little2003combinatorial}.
...
\subsubsection*{Monk moves}
Monk's rule in the form
\begin{equation}\label{eq:monk}
x_\alpha\sch_w+\sum_{\substack{k<\alpha\\ wt_{k\alpha}\gtrdot w}}\sch_{wt_{k\alpha}}=\sum_{\substack{l>\alpha\\ wt_{\alpha l}\gtrdot w}}\sch_{wt_{\alpha l}}
\end{equation}
is realized bijectively on RC graphs by Billey--Holroyd--Young \cite{billey2019bijective} and on bumpless pipe dreams by Huang \cite{huang2020monk}: there are maps
$$x_\alpha\!\rightsquigarrow:\RC(w)\to\bigsqcup_{l}\RC(wt_{\alpha l})\text{,}\qquad m_{k,\alpha}:\RC(wt_{k\alpha})\to\bigsqcup_{l}\RC(wt_{\alpha l})$$
($l$ ranging over the right side of \eqref{eq:monk}), which together form a bijection from the disjoint union of the sets on the left of \eqref{eq:monk} to the disjoint union of those on the right, with $x_\alpha\!\rightsquigarrow$ adding one crossing in row $\alpha$ and $m_{k,\alpha}$ preserving the number of crossings in every row, and maps with the same names and properties (with blank tiles in place of crossings) on bumpless pipe dreams \cite[Definitions~4.2 and~4.4]{GaoHuang2023}. Gao and Huang prove that $\Phi$ intertwines them \cite[Theorem~4.5]{GaoHuang2023}:
\begin{equation}\label{eq:GHintertwine}
\Phi(x_\alpha\!\rightsquigarrow R)=x_\alpha\!\rightsquigarrow\Phi(R)\text{,}\qquad \Phi(m_{k,\alpha}R)=m_{k,\alpha}\Phi(R)\text{.}
\end{equation}
We recall the two descriptions to the extent needed. On RC graphs \cite[Definition~4.2]{GaoHuang2023}, $x_\alpha\!\rightsquigarrow R$ is computed by placing a crossing at the leftmost elbow of row $\alpha$; whenever the crossing just placed makes two pipes cross twice, the other crossing of that pair of pipes is replaced by an elbow and a crossing is placed at the nearest elbow to its right in its row, until the result is reduced. For $D\in\RC(wt_{k\alpha})$, $m_{k,\alpha}D$ is computed in the same way after first replacing by an elbow the unique crossing $c\in D$ with $\rtt_D(c)=(k,\alpha)$, i.e.\ the crossing whose removal yields $w$, and placing a crossing at the nearest elbow to its right in its row. Consequently, the inverse of the Monk bijection on an element $X$ of the right side is computed by removing the crossing $c\in X$ whose removal yields $w$, placing a crossing at the nearest elbow to its left in the same row, and whenever this makes two pipes cross twice, removing the other crossing of that pair and placing a crossing at the nearest elbow to its left in its row; if a crossing to be moved has no elbow to its left, it is simply removed and the process stops, and the result lies in $\RC(w)$ (this is the $x_\alpha\!\rightsquigarrow$ case). Moving a crossing one column to the left decrements its letter by one, and moving it across a block of crossings to the nearest elbow decrements the letters of the block one after the other, the two deletable positions of the intermediate words being adjacent equal letters; hence, when no crossing is removed (the $m_{k,\alpha}$ case), the inverse Monk move is, in terms of the reading word, the Little bump $\ltb_c$ of Definition~\ref{definition:littlebump} at the position of $c$, with the row sequence unchanged. We call it the \emph{Little bump of $X$ at $c$} in either case. Here we use Little's lemma \cite[Lemma~4]{little2003combinatorial} in the following form: if a word $\mathbf c$ is not reduced but $\mathbf c\setminus k$ is, then exactly one pair of wires of $\mathbf c$ crosses twice, the positions whose deletion leaves a reduced word are exactly the two crossings of this pair, and since the right root of a position records the two wires crossing there, these two positions have the same right root up to order.

On bumpless pipe dreams \cite[Algorithm~1 and proof of Theorem~2]{huang2020monk}, $x_\alpha\!\rightsquigarrow B$ is computed by repeatedly modifying the pipe $p$ that leaves through row $\alpha$: $p$ is drooped from its current $r$-shaped turn into the nearest available tile to the southeast, and when the drooped pipe meets a pipe $q$ in a bump tile the bump is either replaced by a crossing (if $p$ and $q$ do not cross, in which case the algorithm stops) or exchanged with the existing crossing of $p$ and $q$, after which $p$ is drooped again. For $B\in\mathrm{BPD}(wt_{k\alpha})$, $m_{k,\alpha}B$ is computed by replacing the crossing of the two pipes that leave through rows $k$ and $\alpha$ by a bump tile, after which the pipe leaving through row $\alpha$ has the $r$-shaped turn in this tile and is the pipe $p$ to which the same algorithm is applied. In both cases the permutation of the diagram is $w$ throughout, except for the final step, so $p$ leaves through row $\alpha$ throughout.

\begin{lemma}[Row confinement]\label{lemma:bpdconfine}
Let $B\in\mathrm{BPD}(w)$ and $B'\in\mathrm{BPD}(wt_{k\alpha})$ with $k<\alpha$ and $wt_{k\alpha}\gtrdot w$. Then $x_\alpha\!\rightsquigarrow B$ agrees with $B$ in rows $1,\ldots,\alpha-1$, and $m_{k,\alpha}B'$ agrees with $B'$ in rows $1,\ldots,\alpha-1$.
\end{lemma}
\begin{proof}
Every tile changed by Huang's algorithm lies either on the pipe $p$ that leaves through row $\alpha$ (the tiles of a bump and of a crossing that are exchanged, the final bump replaced by a crossing, and the initial crossing replaced by a bump, which lies on both pipes leaving through rows $k$ and $\alpha$) or in the rectangle spanned by a droop of $p$, whose northwest corner is a turn of $p$. Since $p$ travels north and east and leaves through row $\alpha$, all of $p$ lies in rows $\geq\alpha$, and so does every such rectangle.
\end{proof}

\begin{lemma}[Little bumps and top rows]\label{lemma:bumpconfine}
Let $X$ be an RC graph with $\wof X=\tau$ and let $c\in X$ be a crossing with $\rtt_X(c)=(s,\beta)$, $s<\beta$, such that $X\setminus\{c\}$ is reduced. Let $X'$ be the RC graph obtained from $X$ by the Little bump at $c$ (the inverse Monk move described above). Then $\Phi(X)$ and $\Phi(X')$ agree in rows $1,\ldots,s-1$.
\end{lemma}
\begin{proof}
Removing $c$ yields $\pi:=\tau t_{s\beta}$ with $\ell(\pi)=\ell(\tau)-1$, so $X\in\RC(\pi t_{s\beta})$ lies on the right side of the Monk bijection for $(\pi,s)$, and $X'$ is its preimage: $X=x_s\!\rightsquigarrow X'$ with $X'\in\RC(\pi)$, or $X=m_{k,s}X'$ with $X'\in\RC(\pi t_{ks})$ for some $k<s$. By \eqref{eq:GHintertwine} the same relation holds between $\Phi(X)$ and $\Phi(X')$, and Lemma~\ref{lemma:bpdconfine} gives the claim.
\end{proof}

\begin{lemma}[Bounded completion]\label{lemma:bpdcompletion}
Let $B\in\mathrm{BPD}(w)$ have no blank tiles in rows $>p$, and suppose $\maxd(w)\leq p$. Then rows $>p$ of $B$ contain no $j$-shaped tiles. Consequently, in rows $>p$ every pipe $c$ travels straight north along column $c$, until it reaches row $p$ or until it reaches its exit row $w^{-1}(c)>p$, where it turns east and travels straight to the east edge; in particular $B$ is determined by $B_{\leq p}$.
\end{lemma}
\begin{proof}
Suppose $(i,c)$ is a $j$-shaped tile with $i>p$, and take $i$ maximal. No pipe enters $(i,c)$ from the south, so pipe $c$ turns east at some tile $(i'',c)$ with $i''>i$; as rows $>i$ contain no $j$-shaped tiles and no blanks, pipe $c$ then travels east along row $i''$ to the edge, so $w(i'')=c$. The pipe $P$ entering $(i,c)$ from the west travels along row $i$ from an $r$-shaped turn $(i,c_0)$, $c_0<c$. The pipe leaving through row $i$ is not $P$; its last turn is an $r$-shaped tile $(i,c_1)$ from which it travels east along row $i$ to the edge. The tiles $(i,c_0),\ldots,(i,c)$ are occupied by $P$ and an $r$-shaped tile cannot be entered from the west, so $c_1>c$. Below row $i$ there are no $j$-shaped tiles, so this pipe traveled straight north along column $c_1$ and is pipe $c_1$: $w(i)=c_1>c=w(i'')$ with $p<i<i''$, contradicting $\maxd(w)\leq p$. The remaining assertions follow: with no blanks and no $j$-shaped tiles in rows $>p$, a pipe in these rows can only travel north in its own column and, after one $r$-shaped turn, east.
\end{proof}

\begin{proposition}[Top rows]\label{proposition:toprows}
Let $R$ be an RC graph and $p\geq 0$. Then $\Phi(R)$ and $\Phi(R_{\leq p})$ agree in rows $1,\ldots,p$.
\end{proposition}
\begin{proof}
It suffices to prove: if $\alpha$ is the last nonempty row of $R$ and $x$ is the leftmost crossing in row $\alpha$, then $\Phi(R)$ and $\Phi(R\setminus\{x\})$ agree in rows $1,\ldots,\alpha-1$. Indeed, removing the crossings of rows $>p$ one at a time in this manner never changes rows $1,\ldots,p$ of $\Phi$ and ends at $R_{\leq p}$.

Let $x=(\alpha,j)$, $a=\alpha+j-1$ its letter, $R^-=R\setminus\{x\}$ and $\pi=\wof{R^-}$. As $x$ is the last letter of the reading word, $R^-$ is reduced and $\wof R=\pi s_a$, so $\rtt_R(x)=(a,a+1)$. We induct on $j$. If $j=1$ then $a=\alpha$, the leftmost elbow of $R^-$ in row $\alpha$ is $(\alpha,1)$, and $R=x_\alpha\!\rightsquigarrow R^-$ because $R$ is reduced; by \eqref{eq:GHintertwine} and Lemma~\ref{lemma:bpdconfine}, $\Phi(R)$ and $\Phi(R^-)$ agree in rows $<\alpha$.

Let $j\geq 2$ and let $X'$ be the Little bump of $R$ at $x$. Since $a>\alpha$, row $a$ of $R$ is empty, so $R$ is not in the image of $x_a\!\rightsquigarrow$; hence $R=m_{k,a}X'$ for some $k<a$, and $X'$ has as many crossings as $R$ in every row. By Lemma~\ref{lemma:bumpconfine}, $\Phi(R)$ and $\Phi(X')$ agree in rows $<a$, hence in rows $<\alpha$. During the bump, a crossing moves only within its row and only to the nearest elbow on its left, so the crossings of row $\alpha$ other than $x$ remain to the right of $x$: the last nonempty row of $X'$ is $\alpha$, its leftmost crossing is $x'=(\alpha,j')$ with $j'<j$, and $X'\setminus\{x'\}$ is reduced. By induction, $\Phi(X')$ and $\Phi(X'\setminus\{x'\})$ agree in rows $<\alpha$. It remains to show that $\Phi(X'\setminus\{x'\})$ and $\Phi(R^-)$ agree in rows $<\alpha$.

Write the reading word of $R$ as $\mathbf a\,a$ with $\mathbf a$ the reading word of $R^-$, and follow the Little bump letter by letter. We use the following restriction property, in which $\mathbf a\mathbf b$ is a word that is not reduced but becomes reduced when a position $k$ of $\mathbf a$ is deleted, and $\mathbf a^-$ denotes $\mathbf a$ with the letter at $k$ decremented: (i) if $\mathbf a^-$ is not reduced, then its defect (the unique position $j\neq k$ with $\mathbf a^-\setminus j$ reduced) is also the defect of $\mathbf a^-\mathbf b$; (ii) if $\mathbf a^-$ is reduced, then $\mathbf a^-\mathbf b$ is reduced or its defect lies in $\mathbf b$. For (i), the exchange property applied to the reduced word $\mathbf a^-\setminus k=\mathbf a\setminus k$ gives $\wof{\mathbf a^-\setminus j}=\wof{\mathbf a\setminus k}$, so $(\mathbf a^-\setminus j)\mathbf b$ has the permutation and the length of the reduced word $(\mathbf a\setminus k)\mathbf b$ and is reduced, and $j$ is the defect of $\mathbf a^-\mathbf b$ by uniqueness. For (ii), a defect $j$ of $\mathbf a^-\mathbf b$ in $\mathbf a$ would give $\wof{\mathbf a^-\setminus j}=\wof{\mathbf a\setminus k}=\wof{\mathbf a^-\setminus k}$, so the distinct positions $j,k$ of the reduced word $\mathbf a^-$ would have the same right root. The same holds with the roles of prefix and suffix exchanged.

The bump begins by decrementing the last letter, $a\mapsto a-1$. If the word is reduced we are done with $X'\setminus\{x'\}=R^-$. Otherwise the defect $k$ lies in $\mathbf a$ by (ii) for the suffix, and by Little's lemma the positions $k$ and the last position have the same right root up to order; the last letter $a-1$ has right root $(a-1,a)$, so $\rtt_{\mathbf a}(k)=\{a-1,a\}$ as a set, and $\mathbf a\setminus k$ is reduced, being a prefix of the reduced word $(\mathbf a\,(a-1))\setminus k$. By (i) and (ii) the bump now performs the complete Little bump of $\mathbf a$ at $k$, producing a reduced word $\mathbf a^{(1)}$ (the RC graph $A^{(1)}$, with the rows of $R^-$), and then either stops or continues by decrementing the last letter again, to $a-2$, after which the same analysis applies to $\mathbf a^{(1)}(a-2)$, and so on. Thus $X'\setminus\{x'\}=A^{(m)}$ where $A^{(0)}=R^-$ and $A^{(i+1)}$ is the Little bump of $A^{(i)}$ at a crossing with right root $\{a-i-1,a-i\}$ whose removal leaves a reduced word. The last letter is decremented to $a-i-1$ only if $x$ moves to column $j-i-1\geq 1$, so $a-i-1\geq\alpha$. By Lemma~\ref{lemma:bumpconfine}, $\Phi(A^{(i)})$ and $\Phi(A^{(i+1)})$ agree in rows $<a-i-1$, hence in rows $<\alpha$, for every $i$, and therefore so do $\Phi(R^-)$ and $\Phi(X'\setminus\{x'\})$.
\end{proof}

\subsubsection*{Transition steps}
\begin{definition}
...
By Proposition~\ref{proposition:pullindex} with $i=n$ (this is the identity used in the proof of Theorem~\ref{theorem:zero_bijection}),
\begin{equation}\label{E:Tn}
\sch_w(x_1,\dots,x_{n-1},0)=\sum_{w'\in T_n(w)}\sch_{w'}(x_1,\dots,x_{n-1}),
\end{equation}
and by Theorem~\ref{theorem:modactrc} together with Corollary~\ref{corollary:evschub} (or Proposition~\ref{proposition:pullindex} with $i=1$, $y=0$),
...
In particular, if $R\in\RC(w,n)^0$ with $n\ge2$, then $\trm(R)\in\RC(v,n-1)^0$ with $v=\wof{\trm R}\in P_1(w)$;
...
so $w=c\cdot{\uparrow}\wof{\trm R}$ with lengths adding, where $c$ is the permutation of row $1$; and $w\downvar{1}\wof{\trm R}$
by the proof of Theorem~\ref{theorem:modactrc}. Row $n-1$ of $\trm R$ is row $n$ of $R$, which is empty.
The last statement follows from Lemmas~\ref{lemma:d1desc} and~\ref{lemma:factor}, which give $w=c\cdot{\uparrow}v$ with lengths adding for every $v\in P_1(w)$.
\end{proof}
...
\subsubsection*{The map $\zeromap$}

\begin{definition}\label{definition:zeromap}
Let $R$ be a bounded RC graph of height $n\geq 1$ whose row $n$ is empty, and let $w=\wof R$. If $\maxd(w)<n$, let $\zeromap(R)$ be $R$ with its last row deleted, a bounded RC graph of height $n-1$. If $\maxd(w)=n$, let $s=\max\{j>n: w(j)<w(n)\}$ and let $c\in R$ be the unique crossing with $\rtt_R(c)=(n,s)$. Let $R^{(1)}$ be the RC graph obtained from $R$ by the Little bump at $c$, i.e.\ the inverse Monk move, and define $\zeromap(R)=\zeromap(R^{(1)})$, where $R^{(1)}$ is regarded as a bounded RC graph of height $n$.
\end{definition}

\begin{lemma}\label{lemma:zeroweight}
Definition~\ref{definition:zeromap} is well posed: for $R$ as in the definition with $\maxd(w)=n$, the crossing $c$ exists and is unique, $R\setminus\{c\}$ is reduced, $R^{(1)}$ is a bounded RC graph of height $n$ with row $n$ empty, $\wtt(R^{(1)})=\wtt(R)$, and $\wof{R^{(1)}}=w\,t_{ns}t_{in}$ is a transition step at $n$ for some $i<n$. The recursion terminates, and $\zeromap(R)$ is a bounded RC graph of height $n-1$ with $\wtt(\zeromap R)=\wtt(R)$.
\end{lemma}
\begin{proof}
Since $\maxd(w)=n$ we have $w(n+1)<w(n+2)<\cdots$, so $(n,s)$ is an inversion of $w$ and $w(k)<w(s)$ for $n<k<s$; hence $wt_{ns}\lessdot w$. The right roots of the positions of a reduced word are its inversions, each occurring once, which gives $c$, and $R\setminus\{c\}\in\RC(wt_{ns})$ is reduced. Put $\pi=wt_{ns}$. Then $R\in\RC(\pi t_{ns})$ lies on the right side of the Monk bijection \eqref{eq:monk} for $(\pi,n)$. As row $n$ of $R$ is empty, $R$ is not of the form $x_n\!\rightsquigarrow D$, so $R=m_{i,n}(R^{(1)})$ for a unique $i<n$ with $\pi t_{in}\gtrdot\pi$ and a unique $R^{(1)}\in\RC(\pi t_{in})$, namely the inverse Monk move, which has the same number of crossings as $R$ in every row; in particular no crossing is removed, so it is the Little bump at $c$. By Lemma~\ref{L:leaves}, $\maxd(wt_{ns}t_{in})\leq n$, so $R^{(1)}$ is a bounded RC graph of height $n$ with row $n$ empty, and the number of positions $j>n$ with $u(j)<u(n)$ strictly decreases along the recursion, which therefore terminates at a permutation with $\maxd<n$.
\end{proof}

\begin{lemma} \label{lemma:zerotransition}
Let $R$ be a bounded RC graph with $\hr(R)=n\geq 1$ such that row $n$ is empty. Then $\wof{R}\downvar{n}\wof{\zeromap R}$ and $\ell(\wof{\zeromap R})=\ell(\wof R)$, i.e.\ $\wof{\zeromap R}\in T_n(\wof R)$.
\end{lemma}
\begin{proof}
By Lemma~\ref{lemma:zeroweight}, $\wof{\zeromap R}$ is reached from $\wof R$ by a sequence of transition steps at $n$ and has $\maxd<n$; by Lemma~\ref{L:leaves} it lies in $T_n(\wof R)$.
\end{proof}

\begin{proposition}[$\zeromap$ erases the last row]\label{proposition:zerotrunc}
Let $R$ be a bounded RC graph of height $n$ whose last row is empty. Then $\Phi(\zeromap R)$ and $\Phi(R)$ agree in rows $1,\ldots,n-1$. Consequently $\zeromap(R)$ is the unique bounded RC graph $S$ of height $n-1$ such that $\Phi(S)$ agrees with $\Phi(R)$ in rows $1,\ldots,n-1$.
\end{proposition}
\begin{proof}
Each step $R\mapsto R^{(1)}$ of Definition~\ref{definition:zeromap} is a Little bump at a crossing with right root $(n,s)$ whose removal leaves a reduced word, so by Lemma~\ref{lemma:bumpconfine} it does not change rows $1,\ldots,n-1$ of $\Phi$; deleting the empty last row does not change $\Phi$ at all. For the second statement, if $S$ is a bounded RC graph of height $n-1$ then $\Phi(S)$ has no blank tiles in rows $>n-1$ and $\maxd(\wof S)\leq n-1$, so $\Phi(S)$ is determined by its rows $1,\ldots,n-1$ by Lemma~\ref{lemma:bpdcompletion}, and $S$ is determined by $\Phi(S)$.
\end{proof}
...
\begin{tikzpicture}[scale=0.7]
  \draw[lightgray, very thin] (0,0) -- (4,0);
  \draw[lightgray, very thin] (0,0) -- (0,4);
  \draw[lightgray, very thin] (0,1) -- (4,1);
  \draw[lightgray, very thin] (1,0) -- (1,4);
  \draw[lightgray, very thin] (0,2) -- (4,2);
  \draw[lightgray, very thin] (2,0) -- (2,4);
  \draw[lightgray, very thin] (0,3) -- (4,3);
  \draw[lightgray, very thin] (3,0) -- (3,4);
  \draw[lightgray, very thin] (0,4) -- (4,4);
  \draw[lightgray, very thin] (4,0) -- (4,4);
  \fill[gray!20] (0.1,3.1) rectangle (0.9,3.9);
  \fill[gray!20] (1.1,3.1) rectangle (1.9,3.9);
  \draw[blue, line width=1.0pt] (2.5,3) .. controls (2.5,3.3) and (2.7,3.5) .. (3,3.5);
  \draw[blue, line width=1.0pt] (3,3.5) -- (4,3.5);
  \fill[gray!20] (0.1,2.1) rectangle (0.9,2.9);
  \draw[blue, line width=1.0pt] (1.5,2) .. controls (1.5,2.3) and (1.7,2.5) .. (2,2.5);
  \draw[blue, line width=1.0pt] (2,2.5) .. controls (2.3,2.5) and (2.5,2.7) .. (2.5,3);
  ...
  \draw[blue, line width=1.0pt] (3,1.5) -- (4,1.5);
  \draw[blue, line width=1.0pt] (3.5,1) -- (3.5,2);
  \draw[blue, line width=1.0pt] (0.5,0) -- (0.5,1);
  \draw[blue, line width=1.0pt] (1.5,0) .. controls (1.5,0.3) and (1.7,0.5) .. (2,0.5);
  \draw[blue, line width=1.0pt] (2,0.5) -- (3,0.5);
  \draw[blue, line width=1.0pt] (2.5,0) -- (2.5,1);
  \draw[blue, line width=1.0pt] (3,0.5) -- (4,0.5);
  \draw[blue, line width=1.0pt] (3.5,0) -- (3.5,1);
  \node[font=\scriptsize] at (0.5,-0.35) {1};
  \node[font=\scriptsize] at (1.5,-0.35) {2};
  \node[font=\scriptsize] at (2.5,-0.35) {3};
  \node[font=\scriptsize] at (3.5,-0.35) {4};
  \node[font=\scriptsize, anchor=west] at (4.15,3.5) {1};
  \node[font=\scriptsize, anchor=west] at (4.15,2.5) {4};
  \node[font=\scriptsize, anchor=west] at (4.15,1.5) {3};
  \node[font=\scriptsize, anchor=west] at (4.15,0.5) {2};
  \draw[black, line width=1pt] (0,0) rectangle (4,4);
\end{tikzpicture}
\caption*{(a) $\Phi(R)$, $\wof R=1432$}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
\centering
\begin{tikzpicture}[scale=0.7]
  ...
  \draw[blue, line width=1.0pt] (3.5,2) .. controls (3.5,2.3) and (3.7,2.5) .. (4,2.5);
  \draw[blue, line width=1.0pt] (0.5,1) .. controls (0.5,1.3) and (0.7,1.5) .. (1,1.5);
  \draw[blue, line width=1.0pt] (1,1.5) -- (2,1.5);
  \draw[blue, line width=1.0pt] (1.5,1) -- (1.5,2);
  \draw[blue, line width=1.0pt] (2,1.5) -- (3,1.5);
  \draw[blue, line width=1.0pt] (3,1.5) -- (4,1.5);
  \draw[blue, line width=1.0pt] (3.5,1) -- (3.5,2);
  \draw[blue, line width=1.0pt] (0.5,0) -- (0.5,1);
  \draw[blue, line width=1.0pt] (1.5,0) -- (1.5,1);
  \draw[blue, line width=1.0pt] (2.5,0) .. controls (2.5,0.3) and (2.7,0.5) .. (3,0.5);
  \draw[blue, line width=1.0pt] (3,0.5) -- (4,0.5);
  \draw[blue, line width=1.0pt] (3.5,0) -- (3.5,1);
  \node[font=\scriptsize] at (0.5,-0.35) {1};
  \node[font=\scriptsize] at (1.5,-0.35) {2};
  \node[font=\scriptsize] at (2.5,-0.35) {3};
  \node[font=\scriptsize] at (3.5,-0.35) {4};
  \node[font=\scriptsize, anchor=west] at (4.15,3.5) {2};
  \node[font=\scriptsize, anchor=west] at (4.15,2.5) {4};
  \node[font=\scriptsize, anchor=west] at (4.15,1.5) {1};
  \node[font=\scriptsize, anchor=west] at (4.15,0.5) {3};
  \draw[black, line width=1pt] (0,0) rectangle (4,4);
\end{tikzpicture}
\caption*{(b) $\Phi(\zeromap R)$, $\wof{\zeromap R}=2413$}
\end{minipage}
\caption{The bumpless pipe dreams of the RC graph $R=\{(1,2),(1,3),(2,2)\}$ of Figure~\ref{fig:zeroing} and of $\zeromap(R)$, drawn on a $4\times4$ grid; blank tiles are shaded and the pipe leaving through each row is labeled at the right. Row $i$ has as many blank tiles as row $i$ of the RC graph has crossings. The two diagrams agree in rows $1$ and $2$ (Proposition~\ref{proposition:zerotrunc}); rows $3$ and $4$ of $\Phi(\zeromap R)$ contain no blank tile and no $j$-tile, so every pipe there travels straight north and then straight east (Lemma~\ref{lemma:bpdcompletion}).}
\label{fig:bpdzero}
\end{figure}

We also record the behavior of $\Phi$ under deleting the first row. Gao and Huang define $\Phi^{-1}$ by iterating an operation $\nabla$ on bumpless pipe dreams \cite[Definitions~3.1 and~3.3]{GaoHuang2023}: if $B=\Phi(R)$ and $x$ is the first crossing of $R$ in reading order (the rightmost crossing of the first nonempty row $r$, with letter $a$), then $\nabla B=\Phi(R\setminus\{x\})$, and the pair $(a,r)$ is read off from the computation of $\nabla B$ as described below; $\Phi(R)$ is the unique diagram whose successive pairs are the reading word and row sequence of $R$. The operation $\nabla B$ is computed as follows \cite[Definition~3.1, Figures~2 and~3]{GaoHuang2023}. A mark is placed on the rightmost blank tile of the first row of $B$ containing a blank tile, and a sequence of \emph{column moves} follows, each of which modifies only a rectangle of width two whose northwest corner $(x,y)$ is the current position of the mark, moved first to the rightmost blank tile of its block of consecutive blank tiles. The tile $(x,y+1)$ carries a pipe $p$ traveling north. If $p$ entered column $y+1$ at a $j$-tile $(x',y+1)$, $x'>x$, the rectangle has southeast corner $(x',y+1)$, and afterwards the mark is at $(x',y+1)$; otherwise $p$ is pipe $y+1$, the rectangle has southeast corner $(x',y+1)$ where $(x',y)$ is the $r$-tile at which pipe $y$ turns east, this is the last column move, and $a=y$. In both cases, in each row $z$ with $x<z<x'$ the pair of tiles $(z,y),(z,y+1)$ is replaced by a pair depending only on the tile $(z,y)$: a blank, horizontal, $r$- or $j$-tile at $(z,y)$ is replaced by a vertical, crossing, vertical, or crossing tile, respectively, and $(z,y+1)$ by a blank, horizontal, $r$- or $j$-tile, respectively, while a vertical or crossing tile at $(z,y)$ leaves the pair unchanged. The corner $(x,y)$ becomes an $r$-tile and $(x,y+1)$ becomes a horizontal or $j$-tile according as it was an $r$-tile or vertical. In the first case $(x',y+1)$ becomes blank and $(x',y)$ becomes vertical or a $j$-tile according as it was an $r$-tile or horizontal; in the second case $(x',y)$ becomes vertical and $(x',y+1)$ an $r$-tile, which is the same replacement as the row rule applied to the $r$-tile $(x',y)$.

\begin{lemma}[Locality of $\nabla$]\label{lemma:poplocal}
Let $B$ be a bumpless pipe dream with at least one blank tile and let $m\geq 1$. Then rows $1,\ldots,m$ of $\nabla B$ are determined by rows $1,\ldots,m$ of $B$. Moreover, if $B'$ is obtained from $B$ by adding a new first row and a new first column, with pipe $1$ traveling north along the new column and east along the new row and all other tiles shifted by $(1,1)$, then $\nabla B'$ is obtained from $\nabla B$ in the same way, and the pair $(a,r)$ read off from $B'$ is the pair read off from $B$ plus $(1,1)$.
\end{lemma}
\begin{proof}
If rows $1,\ldots,m$ of $B$ contain no blank tile, the mark starts below row $m$ and every rectangle lies in rows $>m$. Otherwise the initial position of the mark is determined by $B_{\leq m}$. Consider a column move with northwest corner $(x,y)$, $x\leq m$. Scan column $y+1$ downward from row $x+1$ through rows $\leq m$. If a $j$-tile is met, in row $x'\leq m$, we are in the first case with this $x'$ (in the second case column $y+1$ carries pipe $y+1$ straight north), and the rectangle, its modification and the new position of the mark are determined by $B_{\leq m}$. If only vertical and crossing tiles are met, then in rows $x+1,\ldots,m$ the two columns are modified by the row rule: this is so in the first case, where $x'>m$; in the second case with $x'>m$; and in the second case with $x'\leq m$, because the corner replacement at row $x'$ agrees with the row rule and rows $x'+1,\ldots,m$ of column $y$ carry pipe $y$ vertically, so the row rule leaves them unchanged. In all these cases the mark is afterwards below row $m$ or the computation ends, so rows $1,\ldots,m$ are not changed again. For the second statement, the new row and column contain no blank tile and are never entered by a rectangle, since rectangles extend south and east from the mark, so every column move in $B'$ is the translate of the corresponding column move in $B$, and the last rectangle's left column index and the first blank row both increase by one.
\end{proof}

\begin{lemma} \label{lemma:ztrim}
Let $R$ be a bounded RC graph of height $n\geq 2$ such that row $n$ is empty. Then
$$\zeromap(\trm(R)) = \trm(\zeromap(R))\text{.}$$
\end{lemma}
\begin{proof}
Put $S=\zeromap(R)$, of height $n-1$, and $T=\trm R$, of height $n-1$ with empty last row (Lemma~\ref{L:trim}). Both $\trm S$ and $\zeromap T$ are bounded RC graphs of height $n-2$ (Lemma~\ref{L:trim} and Lemma~\ref{lemma:zeroweight}), so by Proposition~\ref{proposition:zerotrunc} it suffices to show that $\Phi(\trm S)$ and $\Phi(T)$ agree in rows $1,\ldots,n-2$.

Let $X$ be a bounded RC graph of height $n$ with $k$ crossings in row $1$. The first $k$ crossings of $X$ in reading order are those of row $1$, so $\nabla^k\Phi(X)=\Phi(X\setminus\text{row }1)=\Phi({\uparrow}\trm X)$. The reading word and row sequence of ${\uparrow}\trm X$ are those of $\trm X$ with every entry increased by one, so by the second part of Lemma~\ref{lemma:poplocal}, iterated, $\Phi({\uparrow}\trm X)$ is obtained from $\Phi(\trm X)$ by adding a first row and a first column. Hence rows $1,\ldots,n-2$ of $\Phi(\trm X)$ are determined by rows $2,\ldots,n-1$ of $\nabla^k\Phi(X)$, and these are determined by rows $1,\ldots,n-1$ of $\Phi(X)$ by the first part of Lemma~\ref{lemma:poplocal}, applied $k$ times.

Apply this to $X=R$ and to $X=S$, regarded as a bounded RC graph of height $n$ with empty last row; both have $k=\wtt_1(R)$ crossings in row $1$ by Lemma~\ref{lemma:zeroweight}, and $\Phi(S)$ agrees with $\Phi(R)$ in rows $1,\ldots,n-1$ by Proposition~\ref{proposition:zerotrunc}. Therefore $\Phi(\trm S)$ and $\Phi(T)$ agree in rows $1,\ldots,n-2$.
\end{proof}
...
Suppose $y\in S_M$ and $y\downvar{1}u'$, and put $Y=\varphi_{1,M}(u')$. By Definitions~\ref{definition:pieri} and~\ref{definition:polysequence} there are
...
Let $w$ be a permutation with last descent at most $n$. Let $\mathcal{RC}(w,n)^0$ be the set of bounded RC graphs $R$ with $\hr(R) = n$ such that $\wof{R}=w$ and row $n$ is empty. Then
$$\zeromap:\mathcal{RC}(w, n)^{0}\to \mathcal{RC}_{n-1}$$
is injective.
\end{lemma}
\begin{proof}
...
(Theorem~\ref{theorem:modactrc} applied to the empty graph), so $\RC(w,2)^0$ has at most one element.

Let $n\ge3$ and assume the statement for $n-1$ \emph{and every permutation}. Let $R,R'\in\RC(w,n)^0$ with $\zeromap(R)=\zeromap(R')=:S$.
Put $T=\trm R$, $T'=\trm R'$, $v=\wof{T}$, $v'=\wof{T'}$.
\begin{enumerate}
\item By Lemma~\ref{L:trim}, $T\in\RC(v,n-1)^0$ and $T'\in\RC(v',n-1)^0$ with $v,v'\in P_1(w)$. So $\zeromap(T)$, $\zeromap(T')$ are defined and
Lemma~\ref{lemma:ztrim} applies: $\zeromap(T)=\trm S=\zeromap(T')$.
\item Let $u'=\wof{\trm S}$. By Lemma~\ref{lemma:zerotransition}, $v\downvar{n-1}u'$ and $v'\downvar{n-1}u'$; by Lemma~\ref{lemma:zeroweight}, $\wtt(\zeromap(T))=\wtt(T)$ and $\wtt(\zeromap(T'))=\wtt(T')$,
so $\ell(u')=\ell(v)=\ell(v')$. Thus $u'\in T_{n-1}(v)\cap T_{n-1}(v')$.
\item By Lemma~\ref{L:unique}, $v=v'$.
\item Now $T,T'\in\RC(v,n-1)^0$ with $\zeromap(T)=\zeromap(T')$, so $T=T'$ by the inductive hypothesis.
\item By Theorem~\ref{theorem:modactrc}, $R=\mathrm{row}_1(Q_1(v,w))\cup{\uparrow}T=R'$.\qedhere
\end{enumerate}
\end{proof}
...
Note that this sum is without multiplicity, by Lemma~\ref{L:leaves}. The right hand side is equal to
...
$$|\RC(w,n)^0| = \sum_{\substack{w\downvar{n} w'\\\ell(w)=\ell(w')}}|\RC_{n-1}(w')|$$
Now, the set 
$$\{\zeromap(R) \mid R\in \mathcal{RC}(w,n)^0\}$$
has the same cardinality as $\mathcal{RC}(w,n)^0$ by injectivity (Lemma \ref{lemma:zero_injective}), and is a subset of $\bigcup_{w\downvar{n} w',\ \ell(w')=\ell(w)}\mathcal{RC}_{n-1}(w')$ by Lemmas~\ref{lemma:zerotransition} and~\ref{lemma:zeroweight}. Since the right hand side has the same cardinality as $\mathcal{RC}(w,n)^0$, we conclude that $\zeromap$ is surjective, as desired.
\end{proof}
...
We first make the maps $\trm^p$ and $\clp^p$ of the introduction precise. Throughout, $\zeromap^k$ denotes the $k$-fold iterate of $\zeromap$, extended to $\BRC$ as above by declaring $\zeromap(R)=0$ when the last row of $R$ is nonempty. For $a\geq 0$ write $\mathtt{row}(a)=\{(1,1),\ldots,(1,a)\}$ for the bounded RC graph of height $1$ with reading word $s_a s_{a-1}\cdots s_1$.

\begin{lemma}[The last row is determined by its weight]\label{lemma:lastrow}
Let $R$ be a bounded RC graph with $\hr(R)=n\geq 1$ and let $a=\wtt_n(R)$. Then row $n$ of $R$ is $\{(n,1),\ldots,(n,a)\}$, i.e.\ its word is $s_{n+a-1}s_{n+a-2}\cdots s_n$. In particular $\mathtt{row}(a)$ is the unique bounded RC graph of height $1$ with $a$ crossings, and $R$ is determined by $R_{\leq n-1}$ together with $\wtt_n(R)$.
\end{lemma}
\begin{proof}
By Lemma~\ref{L:trim} applied $n-1$ times, $\trm^{n-1}(R)$ is a bounded RC graph of height $1$; its underlying set is $\{(1,j):(n,j)\in R\}$, so it suffices to treat $n=1$. Let $R$ have height $1$ with crossings in columns $j_1>j_2>\cdots>j_a$. Its reading word $s_{j_1}s_{j_2}\cdots s_{j_a}$ is reduced, and $w=\wof{R}$ satisfies $\maxd(w)\leq 1$, so $w(2)<w(3)<\cdots$ and $w$ has code $(a,0,0,\ldots)$, i.e.\ $w=s_a s_{a-1}\cdots s_1$. This permutation is a cycle with a unique reduced word, so $j_k=a+1-k$ for all $k$.
\end{proof}

\begin{definition}\label{definition:cliptrim}
Let $R$ be a bounded RC graph with $\hr(R)=n$ and let $0\leq p\leq n$. For $0\leq k\leq n$ write $R_{\leq k}=\{(i,j)\in R: i\leq k\}$.
\begin{enumerate}
\item $\trm^p(R)=\trm(\trm(\cdots\trm(R)))$ ($p$ times) is the bounded RC graph of height $n-p$ with underlying set $\{(i-p,j):(i,j)\in R,\ i>p\}$. (Each single trim is again a bounded RC graph by Lemma~\ref{L:trim}.)
...
$$\clp^p(R)=\zeromap^{\,h-p}(R_{\leq p})\in\BRC_p\text{.}$$
\end{enumerate}
\end{definition}

For $\clp^p(R)$ to be a single bounded RC graph (rather than $0$) the last row must be empty at every intermediate step. This holds because $\zeromap$ preserves weight vectors (Lemma~\ref{lemma:zeroweight}): if $R'$ has height $h'$ and rows $p+1,\ldots,h'$ empty, then $\zeromap(R')$ has height $h'-1$ and the same weight, so its rows $p+1,\ldots,h'-1$ are empty. Consequently $\clp^p(R)$ is a bounded RC graph of height $p$ with $\wtt(\clp^p(R))=(\wtt_1(R),\ldots,\wtt_p(R))$; it depends only on $R_{\leq p}$, and $\clp^p(R)=\zeromap^{\,N-p}(R_{\leq p})$ for every $N\geq h$, since $\zeromap$ deletes the empty last row whenever the height exceeds $\maxd$. In particular $\clp^p(R)=R_{\leq p}$ whenever $\maxd(\wof{R_{\leq p}})\leq p$; $\clp^0(R)=\emptyset$ (the empty RC graph of height $0$); and $\clp^1(R)=\mathtt{row}(\wtt_1(R))$ by Lemma~\ref{lemma:lastrow}. We use the same formula to define $\clp^p(X)$ for any bounded RC graph $X$ of height $\geq p$, so that $\clp^p(X)=\clp^p(X_{\leq k})$ for all $k\geq p$.
...
Associativity of $\sqcupplus$ rests on two structural identities relating $\clp^p$ and $\trm^p$: a commutativity of $\clp$ with $\trm$, and a functoriality of $\clp$ under iteration.
...
\end{lemma}
\begin{proof}
Since $\trm^p$ is a composite of single trims, it suffices to show $\trm(\clp^{k}(X))=\clp^{k-1}(\trm X)$ for $X$ of height $\geq k\geq 1$. Let $h=\max(k,\maxd(\wof{X_{\leq k}}))$. Iterating Lemma~\ref{lemma:ztrim} (the last row stays empty at each step by weight preservation),
$$\trm(\clp^{k}(X))=\trm\bigl(\zeromap^{h-k}(X_{\leq k})\bigr)=\zeromap^{h-k}\bigl(\trm(X_{\leq k})\bigr)\text{,}$$
and $\trm(X_{\leq k})=(\trm X)_{\leq k-1}$ regarded at height $h-1$, which is bounded by Lemma~\ref{L:trim}. Hence $h-1\geq\max(k-1,\maxd(\wof{(\trm X)_{\leq k-1}}))$ and the right side is $\clp^{k-1}(\trm X)$ by the padding independence noted after Definition~\ref{definition:cliptrim}.
\end{proof}
...
\end{lemma}
\begin{proof}
For bounded RC graphs $X$ of height $\geq p$, Definition~\ref{definition:cliptrim} gives $\clp^p(X)=\zeromap^{\,h-p}(X_{\leq p})$ with $X_{\leq p}$ at height $h\geq p$, each intermediate RC graph having an empty last row. By Propositions~\ref{proposition:zerotrunc} and~\ref{proposition:toprows}, $\Phi(\clp^pX)$ agrees with $\Phi(X_{\leq p})$, and hence with $\Phi(X)$, in rows $1,\ldots,p$. Applying this to $X=\clp^{p+q}(R)$, then to $X=R$ with $p+q$ in place of $p$, and then to $X=R$, we find that $\Phi(\clp^p(\clp^{p+q}R))$ agrees with $\Phi(\clp^{p+q}R)$ in rows $1,\ldots,p$, that $\Phi(\clp^{p+q}R)$ agrees with $\Phi(R)$ in rows $1,\ldots,p+q$, and that $\Phi(R)$ agrees with $\Phi(\clp^pR)$ in rows $1,\ldots,p$. Hence $\Phi(\clp^p(\clp^{p+q}R))$ and $\Phi(\clp^pR)$ agree in rows $1,\ldots,p$. Both $\clp^p(\clp^{p+q}R)$ and $\clp^pR$ are bounded RC graphs of height $p$, so their images under $\Phi$ have no blank tiles in rows $>p$ and permutations with $\maxd\leq p$. By Lemma~\ref{lemma:bpdcompletion} these images are equal, and since $\Phi$ is injective, so are the two RC graphs.
\end{proof}
...
We prove Theorem~\ref{theorem:LR} by comparing two expansions of $\sch_w(x_1,\ldots,x_{p+q})$: the monomial expansion over $\RC_{p+q}(w)$ (Corollary~\ref{corollary:evschub}) and the branching expansion \eqref{eq:branching}. The bridge is the following description of the fibers of $\clp^p$.

For a permutation $u$ and $p\geq 0$, let $h=\max(p,\maxd(u))$ and let $\RC_{\leq p}(u)\subseteq\RC_h(u)$ be the set of RC graphs of $u$ all of whose crossings lie in rows $1,\ldots,p$. For $k\geq 1$ and a permutation $z$ with $\maxd(z)\leq k$ recall $T_k(z)=\{z':z\downvar{k}z',\ \ell(z')=\ell(z)\}$; by \eqref{E:Tn} and linear independence of Schubert polynomials, $T_k(z)=\{z\}$ when $\maxd(z)<k$. For $u'$ with $\maxd(u')\leq p$ let $N_p(u,u')$ be the number of chains
$$u=u_h,\quad u_{h-1}\in T_h(u_h),\quad u_{h-2}\in T_{h-1}(u_{h-1}),\quad\ldots,\quad u_p\in T_{p+1}(u_{p+1}),\qquad u_p=u'\text{,}$$
so that $N_p(u,u')=\delta_{u,u'}$ when $h=p$.

\begin{lemma}\label{lemma:clipfibers}
Let $u$, $p$, $h$ be as above. For every $u'$ with $\maxd(u')\leq p$ and every $U\in\RC_p(u')$,
$$\#\{T\in\RC_{\leq p}(u):\zeromap^{h-p}(T)=U\}=N_p(u,u')\text{.}$$
Consequently
$$\sum_{T\in\RC_{\leq p}(u)}x^{\wtt(T)}=\sch_u(x_1,\ldots,x_p,0,\ldots,0)=\sum_{u'}N_p(u,u')\,\sch_{u'}(x_1,\ldots,x_p)\text{.}$$
\end{lemma}
\begin{proof}
Induct on $h-p$; if $h=p$ then $\zeromap^0$ is the identity and $\RC_{\leq p}(u)=\RC_p(u)$. Suppose $h>p$. Every $T\in\RC_{\leq p}(u)$ has empty last row, so by Theorem~\ref{theorem:zero_bijection} and Lemma~\ref{lemma:zeroweight}, $\zeromap$ restricts to a weight-preserving injection from $\RC_{\leq p}(u)$ into $\bigsqcup_{u_1\in T_h(u)}\RC_{h-1}(u_1)$ (the length condition $\ell(u_1)=\ell(u)$ is forced by weight preservation). By weight preservation the image consists of graphs whose rows $p+1,\ldots,h-1$ are empty, and conversely every such graph $T'\in\RC_{h-1}(u_1)$ is $\zeromap(T)$ for a unique $T\in\RC_h(u)^0$ by Theorem~\ref{theorem:zero_bijection}, and this $T$ has rows $p+1,\ldots,h$ empty by weight preservation, so $T\in\RC_{\leq p}(u)$. Thus $\zeromap$ is a bijection from $\RC_{\leq p}(u)$ onto the set of $T'\in\bigsqcup_{u_1\in T_h(u)}\RC_{h-1}(u_1)$ with rows $p+1,\ldots,h-1$ empty. For such $T'$ with $\wof{T'}=u_1$, put $h_1=\max(p,\maxd(u_1))\leq h-1$; the first $h-1-h_1$ applications of $\zeromap$ to $T'$ delete empty last rows (Definition~\ref{definition:zeromap}), so $\zeromap^{h-1-p}(T')=\zeromap^{h_1-p}(T')$ with $T'$ regarded as an element of $\RC_{\leq p}(u_1)$. Hence
$$\#\{T\in\RC_{\leq p}(u):\zeromap^{h-p}(T)=U\}=\sum_{u_1\in T_h(u)}\#\{T'\in\RC_{\leq p}(u_1):\zeromap^{h_1-p}(T')=U\}=\sum_{u_1\in T_h(u)}N_p(u_1,u')=N_p(u,u')$$
by induction. For the second statement, Corollary~\ref{corollary:evschub} gives $\sum_{T\in\RC_h(u)}x^{\wtt(T)}=\sch_u(x_1,\ldots,x_h)$, and setting $x_{p+1}=\cdots=x_h=0$ kills exactly the terms with a crossing in rows $p+1,\ldots,h$; this is the first equality. For the second, sum the fiber count over $U\in\RC_p(u')$ and all $u'$, using $\wtt(\zeromap^{h-p}(T))=\wtt(T)$ and Corollary~\ref{corollary:evschub} for $\RC_p(u')$.
\end{proof}
...
Let $\Omega$ be the $\mathbb{Z}$-submodule of $\BRC$ generated by the RC graphs $R_1-R_2$ such that $\wof{R_1}=\wof{R_2}$ and $\hr(R_1)=\hr(R_2)$, and let $\mathcal{R}$ be the subring of $\BRC$ generated by the single-row RC graphs $\mathtt{row}(a)$, $a\geq 0$.
\end{definition}

\begin{proof}[Proof of Theorem \ref{theorem:LR}]
Fix $u,v\in S_\infty$ with $\maxd(u)\leq p$, $\maxd(v)\leq q$, and fix $U\in\RC_p(u)$, $V\in\RC_q(v)$. By Lemma~\ref{lemma:dualbranch}, $d_{u,v}^w(p,q)=b_{u,v}^w(p,q)$, so we must show $d_{U,V}^w=b_{u,v}^w(p,q)$ for every $w$ with $\maxd(w)\leq p+q$.

Let $R\in\RC_{p+q}(w)$ and write $R=T\cup\shup^pV'$ with $T=R_{\leq p}$ and $V'=\trm^p(R)\in\RC_q$ (Lemma~\ref{L:trim}). Reading rows from top to bottom, the word of $R$ is the word of $T$ followed by the word of $\shup^pV'$, so
$$w=\wof{T}\cdot\shup^p\wof{V'}\qquad\text{with}\qquad\ell(w)=\ell(\wof T)+\ell(\wof{V'})\text{.}$$
Thus $\wof{T}=u_{v'}:=w\,(\shup^pv')^{-1}$ where $v'=\wof{V'}$, and $\ell(u_{v'})=\ell(w)-\ell(v')$. Conversely, for any $v'$ with $\maxd(v')\leq q$ and $\ell(w(\shup^pv')^{-1})=\ell(w)-\ell(v')$, any $V'\in\RC_q(v')$, and any $T\in\RC_{\leq p}(u_{v'})$, the union $T\cup\shup^pV'$ is a reduced RC graph of $w$ with $p+q$ rows, hence lies in $\RC_{p+q}(w)$. Finally, by Definition~\ref{definition:cliptrim}, $\clp^p(T\cup\shup^pV')=\zeromap^{h-p}(T)$ with $h=\max(p,\maxd(u_{v'}))$.

Applying this with $V'=V$ and Lemma~\ref{lemma:clipfibers},
$$d_{U,V}^w=\#\{T\in\RC_{\leq p}(u_v):\zeromap^{h-p}(T)=U\}=N_p(u_v,u)$$
if $\ell(u_v)=\ell(w)-\ell(v)$, and $d_{U,V}^w=0$ otherwise. On the other hand, by Corollary~\ref{corollary:evschub}, the decomposition $R=T\cup\shup^pV'$, and Lemma~\ref{lemma:clipfibers},
\begin{align*}
\sch_w(x_1,\ldots,x_{p+q})&=\sum_{R\in\RC_{p+q}(w)}x^{\wtt(R)}
=\sum_{\substack{v':\ \maxd(v')\leq q\\ \ell(u_{v'})=\ell(w)-\ell(v')}}\ \sum_{V'\in\RC_q(v')}x_{p+1}^{\wtt_1(V')}\cdots x_{p+q}^{\wtt_q(V')}\sum_{T\in\RC_{\leq p}(u_{v'})}x^{\wtt(T)}\\
&=\sum_{\substack{v':\ \maxd(v')\leq q\\ \ell(u_{v'})=\ell(w)-\ell(v')}}\ \sum_{u'}N_p(u_{v'},u')\,\sch_{u'}(x_1,\ldots,x_p)\,\sch_{v'}(x_{p+1},\ldots,x_{p+q})\text{.}
\end{align*}
Comparing with \eqref{eq:branching} and using the linear independence of the products $\sch_{u'}(x_1,\ldots,x_p)\sch_{v'}(x_{p+1},\ldots,x_{p+q})$ gives $b_{u,v}^w(p,q)=N_p(u_v,u)$ when $\ell(u_v)=\ell(w)-\ell(v)$ and $b_{u,v}^w(p,q)=0$ otherwise. Hence $d_{U,V}^w=b_{u,v}^w(p,q)=d_{u,v}^w(p,q)$, which is independent of the choice of $U$ and $V$ and nonnegative.
\end{proof}

We now record the ring-theoretic consequences.

\begin{definition}
Recall the map $\alpha:\BRC\to\dcoma$, $\alpha(R)=\dsch_{\wof{R}}^{\hr(R)}$. (The weight map $\omega(R)=[\wtt(R)]$ is additive but not multiplicative: every term of $R_1\sqcupplus R_2$ has weight $\wtt(R_1)\wtt(R_2)$, so $\omega(R_1\sqcupplus R_2)$ is a multiple of $\omega(R_1)\sqcupplus\omega(R_2)$ by the number of terms.) Let $a$ be a sequence of elements of a ring indexed by the nonnegative integers. For a permutation $w$ and an integer $n\geq \maxd(w)$, define a function $\dsch_w^n(a)$ by
...
\end{lemma}
\begin{proof}
By Theorem~\ref{theorem:LR}, $\alpha$ is multiplicative: for $R_1\in\RC_p(u)$, $R_2\in\RC_q(v)$,
$$\alpha(R_1\sqcupplus R_2)=\sum_w d_{R_1,R_2}^w\,\dsch_w^{p+q}=\sum_w d_{u,v}^w(p,q)\,\dsch_w^{p+q}=\dsch_u^p\sqcupplus\dsch_v^q=\alpha(R_1)\sqcupplus\alpha(R_2)\text{.}$$
Restricting to $\mathcal{R}$ gives a graded ring map $\alpha|_{\mathcal{R}}:\mathcal{R}\to\dcoma$. Since $\alpha(\mathtt{row}(i))=\dsch^1_{s_i\cdots s_1}=[i]$, the image contains the free generators $[i]$ of $\dcoma$, so $\alpha|_{\mathcal{R}}$ is surjective; by freeness of $\dcoma$ there is a unique graded ring homomorphism $\iota:\dcoma\to\mathcal{R}$ with $\iota([i])=\mathtt{row}(i)$, and $(\alpha|_{\mathcal{R}})\circ\iota=\mathrm{id}_{\dcoma}$, so $\alpha|_{\mathcal{R}}$ is also injective. The family $\{\dsch_w^n\}_{n\ge 0,\, n\ge\maxd(w)}$ is a $\mathbb{Z}$-basis of $\dcoma$ by construction, and $\dsch_w^n(\mathtt{row})=\iota(\dsch_w^n)$ since $\iota$ is a ring homomorphism; applying $\iota$ to the basis gives the claim.
\end{proof}

The section $\iota$ has an explicit description in the RC graph basis.

\begin{lemma}\label{lemma:principalsection}
Let $\maxd(u)\leq n$ and let $P_u\in\RC_n(u)$ be the principal RC graph of $u$, the unique RC graph of $u$ with weight $\code(u)$. Then
$$\iota(\dsch_u^n)=\sum_{R\in\RC_n}c_{\wtt(R),u}\,R\text{,}$$
where $c_{\alpha,u}$ is the coefficient of $\sch_u$ in the Schubert expansion of $x_\alpha$, and the only RC graph of permutation $u$ occurring is $P_u$, with coefficient $1$. In particular $\iota(\dsch_u^n)-P_u\in\Omega$ has no term of permutation $u$.
\end{lemma}
\begin{proof}
We have $\iota([\alpha])=\mathtt{row}(\alpha_1)\sqcupplus\cdots\sqcupplus\mathtt{row}(\alpha_n)=\sum_{\wtt(R)=\alpha}R$: by induction on $n$, the terms of $\mathtt{row}(\alpha_1)\sqcupplus\sum_{\wtt(R')=(\alpha_2,\ldots,\alpha_n)}R'$ are the $R$ with $\trm(R)$ of weight $(\alpha_2,\ldots,\alpha_n)$ and $\clp^1(R)=\mathtt{row}(\alpha_1)$, i.e.\ $\wtt_1(R)=\alpha_1$ (Lemma~\ref{lemma:lastrow}). Since $\dsch_u^n=\sum_\alpha c_{\alpha,u}[\alpha]$ by definition, the formula follows. The Schubert polynomials are unitriangular with respect to the monomials in reverse lexicographic order, $\sch_v=x^{\code(v)}+\text{(lower terms)}$ \cite{notes}, so $c_{\alpha,u}\neq0$ forces $\code(u)\leq\alpha$, while $\alpha=\wtt(R)$ for $R\in\RC_n(u)$ means that $x^\alpha$ occurs in $\sch_u$ (Corollary~\ref{corollary:evschub}), forcing $\alpha\leq\code(u)$. Hence $\alpha=\code(u)$, $R=P_u$, and $c_{\code(u),u}=1$.
\end{proof}

\begin{theorem} \label{theorem:semidirect}
  The map $\alpha:\BRC\to \dcoma$ is a surjective homomorphism of rings with kernel $\Omega$; in particular $\Omega$ is a two-sided ideal of $\BRC$. The ring homomorphism $\iota:\dcoma\to\BRC$ sending $[i]\mapsto \mathtt{row}(i)$ is a right inverse to $\alpha$, with image $\mathcal{R}$, and $\BRC=\mathcal{R}\oplus\Omega$. Consequently $\BRC\cong \dcoma\ltimes \Omega$ as rings.
\end{theorem}
\begin{proof}
By Lemma~\ref{lemma:schubertrow} (whose first sentence is the multiplicativity of $\alpha$), $\alpha$ is a ring homomorphism; it is surjective since every $\dsch_w^n$ is $\alpha(R)$ for any $R\in\RC_n(w)$, and $\RC_n(w)\neq\emptyset$ when $\maxd(w)\leq n$.

The kernel of $\alpha$ is $\Omega$: clearly $\Omega\subseteq\ker\alpha$. Conversely, if $x=\sum_R c_RR\in\ker\alpha$, group the terms by $(\wof{R},\hr(R))$; then $\alpha(x)=\sum_{(w,n)}\bigl(\sum_{R\in\RC_n(w)}c_R\bigr)\dsch_w^n=0$ forces $\sum_{R\in\RC_n(w)}c_R=0$ for each $(w,n)$ by linear independence of the $\dsch_w^n$, and an element $\sum_{R\in\RC_n(w)}c_RR$ with $\sum c_R=0$ lies in $\Omega$. As the kernel of a ring homomorphism, $\Omega$ is a two-sided ideal.

By Lemma~\ref{lemma:schubertrow}, $\iota$ is a ring isomorphism $\dcoma\to\mathcal{R}$ with $\alpha\circ\iota=\mathrm{id}$. The split exact sequence $0\to\Omega\to\BRC\xrightarrow{\alpha}\dcoma\to0$ with section $\iota$ gives $\BRC=\mathcal{R}\oplus\Omega$ as abelian groups, with $\mathcal{R}$ a subring and $\Omega$ a two-sided ideal; this is precisely the statement $\BRC\cong\dcoma\ltimes\Omega$.
\end{proof}
...
\begin{theorem}[{\cite{lascoux1990keys,assaf2018demazure}}] \label{theorem:keykey}
...
whenever the left-hand sides are defined. Moreover, for $R\in\RC(w,n)^0$, $e_i(\zeromap R)$ is defined if and only if $e_i(R)$ is, and likewise for $f_i$; thus $\zeromap$ is an isomorphism of crystals from $\RC(w,n)^0$ onto $\bigcup_{w'\in T_n(w)}\RC(w',n-1)$.
\end{corollary}
\begin{proof}
Insert the letters of the reading word of an RC graph $R$ of height $m$ from right to left by Edelman--Greene insertion \cite{edelman1987balanced}, let $P(R)$ be the insertion tableau, and let $T^-(R)$ be the recording tableau with each entry replaced by the negative of the row index of the corresponding crossing (Figure~\ref{fig:egexample}). Then $T^-(R)$ is a semistandard tableau in English notation (rows weakly increasing to the right, columns strictly increasing downward) in the alphabet $-m<\cdots<-1$, the pair $(P(R),T^-(R))$ determines $R$, and the operators $e_i,f_i$ of Definition~\ref{definition:crystal} correspond to the tableau crystal operators $f_{-(i+1)}$ and $e_{-(i+1)}$ on $T^-(R)$, leaving $P(R)$ unchanged; this is the compatibility of the Morse--Schilling crystal on reduced factorizations with Edelman--Greene insertion \cite{morse2016crystal}, in which the factors are indexed from right to left, whence the reversal of the alphabet. Each step of $\zeromap$ (Definition~\ref{definition:zeromap}) replaces the reading word by its Little bump at the crossing of the two wires ending in positions $n$ and $s$, where $n$ and $s$ depend only on the permutation, and keeps the row sequence. By Theorem~\ref{theorem:littlecrystal} this leaves the recording tableau, hence $T^-(R)$, unchanged, and by \cite[Lemma~3.3]{hamaker2014relating} Little bumps at the crossing of a fixed pair of wires commute with Coxeter--Knuth moves, so the insertion tableau of the bumped word depends only on $P(R)$ \cite[Theorem~6.24]{edelman1987balanced}. Consequently, if $R$ corresponds to $(P,T)$ and $e_i(R)$ to $(P,f_{-(i+1)}T)$, then after each step the two computations of $\zeromap$ correspond to $(P',T)$ and $(P',f_{-(i+1)}T)$ for the same $P'$; in particular the permutations agree at every stage, the computations stop simultaneously, and $\zeromap(e_iR)=e_i\zeromap(R)$. The same argument applies to $f_i$.

For the last statement put $X=\RC(w,n)^0$ and $Y=\bigcup_{w'\in T_n(w)}\RC(w',n-1)$. By Theorem~\ref{theorem:zero_bijection}, $\zeromap$ is a weight-preserving bijection $X\to Y$, and by the first part it maps each $i$-string of $X$ (a maximal set of elements connected by $e_i,f_i$) onto a set of consecutive elements of an $i$-string of $Y$. Since the weights of $X$ and $Y$ agree as multisets, $X$ and $Y$ have the same number of $i$-strings of each length. An $i$-string of maximal length $L$ in $X$ is mapped into an $i$-string of $Y$ of length at least $L$, hence of length $L$, and onto it; distinct such strings of $X$ have distinct images. Removing these strings from $X$ and their images from $Y$ leaves sets with equal weight multisets, and iterating shows that $\zeromap$ maps every $i$-string of $X$ onto an $i$-string of $Y$, which is the claim.
\end{proof}
...
\centering
\raisebox{-0.5\height}{\begin{tikzpicture}[scale=0.6]
  \draw[lightgray, very thin, opacity=0.3] (0,1) -- (4,1);
  \draw[lightgray, very thin, opacity=0.3] (0,2) -- (4,2);
  \draw[lightgray, very thin, opacity=0.3] (0,3) -- (4,3);
  \draw[lightgray, very thin, opacity=0.3] (0,4) -- (4,4);
  ...
  \draw[lightgray, very thin, opacity=0.3] (3,1) -- (3,4);
  \draw[lightgray, very thin, opacity=0.3] (4,1) -- (4,4);
  \draw[blue, line width=1.5pt] (0,3.5) .. controls (0.3,3.5) and (0.5,3.7) .. (0.5,4);
  \draw[blue, line width=1.5pt] (0.5,3) .. controls (0.5,3.3) and (0.7,3.5) .. (1,3.5);
  \draw[blue, line width=1.5pt, line cap=round] (1,3.5) -- (2,3.5);
  \draw[blue, line width=1.5pt, line cap=round] (1.5,3) -- (1.5,4);
  \draw[blue, line width=1.5pt, line cap=round] (2,3.5) -- (3,3.5);
  \draw[blue, line width=1.5pt, line cap=round] (2.5,3) -- (2.5,4);
  \draw[blue, line width=1.5pt] (3,3.5) .. controls (3.3,3.5) and (3.5,3.7) .. (3.5,4);
  \draw[blue, line width=1.5pt] (0,2.5) .. controls (0.3,2.5) and (0.5,2.7) .. (0.5,3);
  \draw[blue, line width=1.5pt] (0.5,2) .. controls (0.5,2.3) and (0.7,2.5) .. (1,2.5);
  \draw[blue, line width=1.5pt, line cap=round] (1,2.5) -- (2,2.5);
  \draw[blue, line width=1.5pt, line cap=round] (1.5,2) -- (1.5,3);
  \draw[blue, line width=1.5pt] (2,2.5) .. controls (2.3,2.5) and (2.5,2.7) .. (2.5,3);
  \draw[blue, line width=1.5pt] (0,1.5) .. controls (0.3,1.5) and (0.5,1.7) .. (0.5,2);
  \draw[blue, line width=1.5pt] (0.5,1) .. controls (0.5,1.3) and (0.7,1.5) .. (1,1.5);
  \draw[blue, line width=1.5pt] (1,1.5) .. controls (1.3,1.5) and (1.5,1.7) .. (1.5,2);
  \node[font=\Large\bfseries, text=green!60!black, fill=white, inner sep=0.5pt, circle, transform shape] at (1.5,3.5) {2};
  \node[font=\Large\bfseries, text=green!60!black, fill=white, inner sep=0.5pt, circle, transform shape] at (2.5,3.5) {3};
  \node[font=\Large\bfseries, text=green!60!black, fill=white, inner sep=0.5pt, circle, transform shape] at (1.5,2.5) {3};
  \node[font=\scriptsize, text=black, anchor=east] at (-0.3,3.5) {1};
  ...
  \draw[black, line width=1.7999999999999998pt] (0,1) rectangle (4,4);
\end{tikzpicture}}\quad
\raisebox{-0.5\height}{\begin{tikzpicture}[scale=1]
  \draw (0.00,0.00) rectangle (0.55,-0.55);
  \node[font=\small] at (0.28,-0.28) {$2$};
  \draw (0.55,0.00) rectangle (1.10,-0.55);
  \node[font=\small] at (0.83,-0.28) {$3$};
  \draw (0.00,-0.55) rectangle (0.55,-1.10);
  \node[font=\small] at (0.28,-0.83) {$3$};
\end{tikzpicture}}\quad
\raisebox{-0.5\height}{\begin{tikzpicture}[scale=1]
  \draw (0.00,0.00) rectangle (0.55,-0.55);
  \node[font=\small] at (0.28,-0.28) {$-2$};
  \draw (0.55,0.00) rectangle (1.10,-0.55);
  \node[font=\small] at (0.83,-0.28) {$-1$};
  \draw (0.00,-0.55) rectangle (0.55,-1.10);
  \node[font=\small] at (0.28,-0.83) {$-1$};
\end{tikzpicture}}
\caption*{(a) $R=\{(1,2),(1,3),(2,2)\}$, reading word $3\,2\,3$ with row sequence $1\,1\,2$; $P(R)$ and $T^-(R)$}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
\centering
\raisebox{-0.5\height}{\begin{tikzpicture}[scale=0.6]
  ...
  \draw[blue, line width=1.5pt, line cap=round] (1,3.5) -- (2,3.5);
  \draw[blue, line width=1.5pt, line cap=round] (1.5,3) -- (1.5,4);
  \draw[blue, line width=1.5pt] (2,3.5) .. controls (2.3,3.5) and (2.5,3.7) .. (2.5,4);
  \draw[blue, line width=1.5pt] (2.5,3) .. controls (2.5,3.3) and (2.7,3.5) .. (3,3.5);
  \draw[blue, line width=1.5pt] (3,3.5) .. controls (3.3,3.5) and (3.5,3.7) .. (3.5,4);
  \draw[blue, line width=1.5pt, line cap=round] (0,2.5) -- (1,2.5);
  \draw[blue, line width=1.5pt, line cap=round] (0.5,2) -- (0.5,3);
  \draw[blue, line width=1.5pt, line cap=round] (1,2.5) -- (2,2.5);
  \draw[blue, line width=1.5pt, line cap=round] (1.5,2) -- (1.5,3);
  \draw[blue, line width=1.5pt] (2,2.5) .. controls (2.3,2.5) and (2.5,2.7) .. (2.5,3);
  \draw[blue, line width=1.5pt] (0,1.5) .. controls (0.3,1.5) and (0.5,1.7) .. (0.5,2);
...
\begin{lemma}\label{lemma:thetaideal}
  $\Theta$ is a two-sided ideal of $\BRC$.
\end{lemma}
\begin{proof}
Let $R_1,R_2$ be bounded RC graphs of height $n$ with $\mathbb{Y}(R_1)=\mathbb{Y}(R_2)$, and let $R$ be a bounded RC graph of height $m$. Since crystal operators preserve the permutation, $\wof{R_1}=\wof{R_2}$.

\emph{Left ideal.} A bounded RC graph $R'$ of height $m+n$ is a term of $R\sqcupplus R_1$ if and only if $\trm^m(R')=R_1$ and $\clp^m(R')=R$. Writing $R'=T\cup\shup^mR_1$ with $T=R'_{\leq m}$, the second condition depends only on $T$ (Definition~\ref{definition:cliptrim}), and $T\cup\shup^mR_1$ is a bounded RC graph if and only if the word of $T$ followed by the word of $\shup^m R_1$ is reduced and the resulting permutation has last descent at most $m+n$; this depends on $R_1$ only through $\wof{R_1}=\wof{R_2}$. Hence $T\cup\shup^mR_1\mapsto T\cup\shup^mR_2$ is a bijection from the terms of $R\sqcupplus R_1$ to those of $R\sqcupplus R_2$. For $i>m$ the operators $e_i$ act on rows $i,i+1$ only, so $e_i(T\cup\shup^mR_1)=T\cup\shup^me_{i-m}(R_1)$, with either side defined if and only if the other is. Applying the raising operators that carry $R_1$ to $\mathbb{Y}(R_1)$ and using that the highest weight of a connected component is unique,
$$\mathbb{Y}(T\cup\shup^mR_1)=\mathbb{Y}(T\cup\shup^m\mathbb{Y}(R_1))=\mathbb{Y}(T\cup\shup^m\mathbb{Y}(R_2))=\mathbb{Y}(T\cup\shup^mR_2)\text{.}$$
Thus $R\sqcupplus R_1-R\sqcupplus R_2=\sum_T\bigl((T\cup\shup^mR_1)-(T\cup\shup^mR_2)\bigr)\in\Theta$.

\emph{Right ideal.} Since $R_1$ and $R_2$ lie in the same connected component, it suffices to treat $R_2=e_i(R_1)$ for some $1\leq i\leq n-1$ (the case $R_2=f_i(R_1)$ is symmetric). For $R'$ of height $n+m$ with $\trm^n(R')=R$, the operator $e_i$ ($i\leq n-1$) acts on rows $i,i+1\leq n$, so $\trm^n(e_iR')=R$ when $e_iR'$ is defined, and $\clp^n(e_iR')=e_i(\clp^nR')$ by Corollary~\ref{corollary:crystaliso} applied to each $\zeromap$ step in Definition~\ref{definition:cliptrim} (erasing rows of index $>n$ commutes with $e_i$, and $\zeromap$ is an isomorphism of crystals onto its image by Corollary~\ref{corollary:crystaliso}, so $e_i$ is defined on $\clp^nR'$ if and only if it is defined on $R'$). Hence $e_i$ restricts to a bijection from the terms of $R_1\sqcupplus R$ onto the terms of $e_i(R_1)\sqcupplus R$, and corresponding terms $R'$, $e_i(R')$ have the same highest weight. Thus $R_1\sqcupplus R-R_2\sqcupplus R\in\Theta$.
\end{proof}
...
Both graphs have permutation $w_c=(1,2,3,5,7,4,10,6,8,9)$. They differ only in their first two rows: $R_1$ has row data $[(6,4),(9,),()]$ in the top three rows, while $R_2$ has $[(9,4),(6,),()]$. Applying $\clp^3$ (retaining the top $3$ rows and applying $\zeromap$ until the height is $3$) to each yields the same $3$-row graph with extremal weight $a=(0,1,2)$:
$$\clp^3(R_1)=\clp^3(R_2)=
...
\end{lemma}
\begin{proof}
That $\Sigma$ is a left ideal follows from the same mechanism used for $\Theta$ (Lemma~\ref{lemma:thetaideal}): the terms of $R\sqcupplus R_1$ and $R\sqcupplus R_2$ are in bijection $T\cup\shup^mR_1\leftrightarrow T\cup\shup^mR_2$, since $\wof{R_1}=\wof{R_2}$, and the crystal operators $e_i$, $i>\hr(R)$, act on the trailing block exactly as on $R_1$, $R_2$. The quasi-Yamanouchi map $\QY$ is defined by applying a sequence of crystal raising operators determined by the weight (\cite{assaf2017slide}); hence applying $\QY$ to $R\sqcupplus R_1$ and to $R\sqcupplus R_2$ produces graphs whose trailing rows have a common partial quasi-Yamanouchi reduction $R\sqcupplus \QY(R_1)=R\sqcupplus \QY(R_2)$, and so $\QY(R\sqcupplus R_1)=\QY(R\sqcupplus R_2)$.
...
\item[$\clp^p(R)$] ``Clip'': the RC graph in $\RC_p$ obtained from the top $p$ rows of $R$ by applying $\zeromap$ until the height is $p$ (Definition~\ref{definition:cliptrim}).
...
We dedicate this section to the otherwise distracting proof of Proposition \ref{proposition:pullindex}. Recall from Definition~\ref{definition:divdiff} that $\delta^i=\frac{1}{y_i-y_{i+1}}(1-s_i)$ acts on the $y$ variables, that $\delta^v\sch_w(x;y)=(-1)^{\ell(v)}\sch_{vw}(x;y)$ whenever $\ell(vw)=\ell(w)-\ell(v)$, and that the skew operators $\delta_u^w$ are characterized by the Leibniz formula $\delta^w(pq)=\sum_{u\leq w}\delta^u(p)\,\delta_u^w(q)$.
...
then
$$\ddv{y}_u^w\prod_{i=1}^k(x_1 - y_i)=(-1)^{\ell(u,w)}\prod_{q\in Q}(x_1 - y_q)$$
\end{proposition2}
\begin{proof}
	The cited theorem, in the special case $p=k$ (the Molev--Sagan Pieri formula for double Schubert polynomials), is stated for the operators $\partial_u^w$ on the $x$ variables applied to $\prod_{i=1}^k(x_i-y_1)$, with value $\prod_{q\in Q}(x_q-y_1)$. Exchanging the roles of $x$ and $y$ gives $\delta_u^w\prod_{i=1}^k(y_i-x_1)=\prod_{q\in Q}(y_q-x_1)$. Since $\prod_{i=1}^k(y_i-x_1)=(-1)^k\prod_{i=1}^k(x_1-y_i)$, $\prod_{q\in Q}(y_q-x_1)=(-1)^{|Q|}\prod_{q\in Q}(x_1-y_q)$, and $|Q|=k-\ell(u,w)$ (the positions $i\leq k$ moved by the $\ell(u,w)$ transpositions witnessing $u\tom{k}w$ are distinct), the stated sign follows.
\end{proof}

\begin{proof}[Proof of Proposition \ref{proposition:pullindex}]
Suppose $v\in S_n$. Since $\ell(vw_0(n))=\ell(w_0(n))-\ell(v)$, we have
$$\sch_v(x;y)=(-1)^{\ell(vw_0(n))}\,\ddv{y}^{vw_0(n)}(\sch_{w_0(n)}(x;y))$$
This is equal to
$$(-1)^{\ell(vw_0(n))}\,\ddv{y}^{vw_0(n)}\left(\sch_{s_{n+1-i}\cdots s_1w_0(n)}(x^{(i)};y)\prod_{j=1}^{n+1-i}(x_i - y_j)\right)$$
and, applying the Leibniz formula together with $\delta^u\sch_{s_{n+1-i}\cdots s_1w_0(n)}(x^{(i)};y)=(-1)^{\ell(u)}\sch_{v'}(x^{(i)};y)$ for $u=v'w_0(n)s_1\cdots s_{n+1-i}$, is also equal to
$$\sum_{\substack{v'\in S_\infty\\\ell(v'w_0(n)s_1\cdots s_{n+1-i})\\=\ell(w_0(n)s_1\cdots s_{n+1-i})-\ell(v')}}(-1)^{\ell(vw_0(n))+\ell(v'w_0(n)s_1\cdots s_{n+1-i})}\,\sch_{v'}(x^{(i)};y)\ddv{y}_{v'w_0(n)s_1\cdots s_{n+1-i}}^{vw_0(n)}\prod_{j=1}^{n+1-i}(x_i - y_j)$$
...
Applying Proposition \ref{proposition:pieri}, the skew operator contributes a further sign $(-1)^{\ell(v'w_0(n)s_1\cdots s_{n+1-i},\,vw_0(n))}$; together with the signs already present this is $(-1)^{2\ell(vw_0(n))}=1$, so the result is equal to
...
\bibliography{schubert_bialgebra,schubert_bialgebra_bpd}
