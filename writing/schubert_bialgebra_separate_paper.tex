...
\end{lemma}
\begin{proof}
For bounded RC graphs $X$ of height $\geq p$, Definition~\ref{definition:cliptrim} gives $\clp^p(X)=\zeromap^{\,h-p}(X_{\leq p})$ with $X_{\leq p}$ at height $h\geq p$, each intermediate RC graph having an empty last row. By Propositions~\ref{proposition:zerotrunc} and~\ref{proposition:toprows},
$$\Phi(\clp^p X)\ \text{agrees with}\ \Phi(X_{\leq p})\ \text{and hence with}\ \Phi(X)\ \text{in rows }1,\ldots,p\text{.}$$
Apply this to $X=\clp^{p+q}(R)$, to $X=R$ with $p+q$ in place of $p$, and to $X=R$: in rows $1,\ldots,p$,
$$\Phi\bigl(\clp^p(\clp^{p+q}R)\bigr)\sim\Phi(\clp^{p+q}R)\sim\Phi(R)\sim\Phi(\clp^pR)\text{,}$$
where the middle agreement holds in rows $1,\ldots,p+q$. Both $\clp^p(\clp^{p+q}R)$ and $\clp^pR$ are bounded RC graphs of height $p$, so their images under $\Phi$ have no blank tiles in rows $>p$ and permutations with $\maxd\leq p$. By Lemma~\ref{lemma:bpdcompletion} these images are equal, and since $\Phi$ is injective, so are the two RC graphs.
\end{proof}
...
\end{corollary}
\begin{proof}
Under Edelman--Greene insertion, an RC graph $R$ corresponds to the pair $(P,T)$, where $P$ is the insertion tableau of the reading word of $R$ and $T$ is the semistandard tableau obtained from the recording tableau by replacing each entry with the row of the corresponding crossing; the operators $e_i,f_i$ of Definition~\ref{definition:crystal} act on $T$ by the usual tableau crystal operators and leave $P$ unchanged \cite{morse2016crystal,assaf2018demazure}. Each step of $\zeromap$ (Definition~\ref{definition:zeromap}) replaces the reading word by its Little bump at the crossing of the two wires ending in positions $n$ and $s$, where $n$ and $s$ depend only on the permutation, and keeps the row sequence. By Theorem~\ref{theorem:littlecrystal} this leaves the recording tableau, hence $T$, unchanged, and by \cite[Lemma~3.3]{hamaker2014relating} Little bumps at the crossing of a fixed pair of wires commute with Coxeter--Knuth moves, so the insertion tableau of the bumped word depends only on $P$ \cite[Theorem~6.24]{edelman1987balanced}. Consequently, if $R$ corresponds to $(P,T)$ and $e_i(R)$ to $(P,e_iT)$, then after each step the two computations of $\zeromap$ correspond to $(P',T)$ and $(P',e_iT)$ for the same $P'$; in particular the permutations agree at every stage, the computations stop simultaneously, and $\zeromap(e_iR)=e_i\zeromap(R)$. The same argument applies to $f_i$.
