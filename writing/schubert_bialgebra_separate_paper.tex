...
\subsection{Functoriality of \texorpdfstring{$\clp$}{clip} via bumpless pipe dreams}\label{section:bpd}

The identity $\clp^p\circ\clp^{p+q}=\clp^p$ is proved by transporting it along the Gao--Huang bijection to bumpless pipe dreams, where erasing rows is literal. Throughout this subsection an RC graph is a finite set $R\subseteq\mathbb P\times\mathbb P$ with $\ell(\wof R)=|R|$ (a reduced pipe dream); the height assigned to it plays no role until Lemma~\ref{lemma:clpfunc}. We write $R_{\leq p}=\{(i,j)\in R:i\leq p\}$ as before.
...
Let $Y$ be a bounded RC graph of height $m$ whose last row is empty. Then $\Phi(\zeromap Y)$ and $\Phi(Y)$ agree in rows $1,\ldots,m-1$.
\end{proposition}
\begin{proof}
If $\maxd(\wof Y)<m$ then $\zeromap Y$ is $Y$ with the empty last row deleted, and $\Phi$ depends only on the set of crossings. Otherwise $\zeromap=\lzeromap$ by Proposition~\ref{proposition:zerocrystal}, and $\lzeromap Y$ is computed by iterating $\lmap$: each step replaces the current RC graph $X$, with $d=\maxd(\wof X)\geq m$ and rows $\geq m$ empty (so that $X$ is a bounded RC graph of height $d$ with empty last row), by the Little bump at the crossing $c$ with $\rtt_X(c)=(d,d+1)$ (Proposition~\ref{proposition:little_bump}); as $d$ is a descent of $\wof X$, $X\setminus\{c\}$ is reduced. By Lemma~\ref{lemma:bumpconfine} each step preserves rows $1,\ldots,d-1\supseteq\{1,\ldots,m-1\}$ of $\Phi$, and the final deletion of empty rows does not change $\Phi$.
\end{proof}
