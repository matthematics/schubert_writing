...
\item Suppose $n\geq 1$. Let $h=\max\bigl(n-1,\maxd(\wof{R_{\leq n-1}})\bigr)$ and regard $R_{\leq n-1}$ as a bounded RC graph of height $h$; its rows $n,\ldots,h$ are empty. The \emph{one-row clip} of $R$ is
$$\clp^{n-1}(R)=\zeromap^{\,h-n+1}(R_{\leq n-1})\in\BRC_{n-1}\text{.}$$
\item Define $\clp^p(R)$ recursively by $\clp^n(R)=R$ and $\clp^p(R)=\clp^p\bigl(\clp^{n-1}(R)\bigr)$ for $p<n$; that is, $\clp^p(R)$ is the result of $n-p$ successive one-row clips.
\end{enumerate}
\end{definition}

For the one-row clip to be a single bounded RC graph (rather than $0$) the last row must be empty at every intermediate step. This holds because $\zeromap$ preserves weight vectors (Lemma~\ref{lemma:zeroweight}): if $R'$ has height $h$ and rows $n,\ldots,h$ empty, then $\zeromap(R')$ has height $h-1$ and the same weight, so its rows $n,\ldots,h-1$ are empty. Consequently $\clp^p(R)$ is a bounded RC graph of height $p$ with $\wtt(\clp^p(R))=(\wtt_1(R),\ldots,\wtt_p(R))$. In particular $\clp^{n-1}(R)=R_{\leq n-1}$ whenever $\maxd(\wof{R_{\leq n-1}})\leq n-1$; $\clp^0(R)=\emptyset$ (the empty RC graph of height $0$); and $\clp^1(R)=\mathtt{row}(\wtt_1(R))$ by Lemma~\ref{lemma:lastrow}.
...
\end{lemma}
\begin{proof}
Both sides are the composite of $n-p$ one-row clips applied to $R$ (Definition~\ref{definition:cliptrim}(3)).
\end{proof}
...
\end{lemma}
\begin{proof}
Both sides have height $q$. Since $\trm^p$ is a composite of single trims and $\clp^{p+q}$, $\clp^q$ are composites of one-row clips, it suffices to show that a single trim commutes with a one-row clip: for $R$ of height $n\geq 2$,
$$\trm(\clp^{n-1}(R))=\clp^{n-2}(\trm(R))\text{.}$$
Let $h=\max(n-1,\maxd(\wof{R_{\leq n-1}}))$, so $\clp^{n-1}(R)=\zeromap^{h-n+1}(R_{\leq n-1})$ with $R_{\leq n-1}$ at height $h$. Then $\trm(R_{\leq n-1})=(\trm R)_{\leq n-2}$ at height $h-1$, and $\maxd(\wof{\trm(R_{\leq n-1})})\leq h-1$ by Lemma~\ref{L:trim}. Set $h'=\max(n-2,\maxd(\wof{(\trm R)_{\leq n-2}}))\leq h-1$, so $\clp^{n-2}(\trm R)=\zeromap^{h'-n+2}((\trm R)_{\leq n-2})$ at height $h'$. Since $\zeromap$ acts on a graph whose permutation has last descent below its height by deleting the empty last row, $\zeromap^{h-1-h'}$ carries $(\trm R)_{\leq n-2}$ at height $h-1$ to the same graph at height $h'$, whence $\clp^{n-2}(\trm R)=\zeromap^{h-n+1}(\trm(R_{\leq n-1}))$. Lemma \ref{lemma:ztrim} gives $\trm(\zeromap(R'))=\zeromap(\trm(R'))$ for any $R'$ with empty last row, and the last row stays empty throughout by weight preservation, so iterating $h-n+1$ times,
$$\trm(\clp^{n-1}(R))=\trm(\zeromap^{h-n+1}(R_{\leq n-1}))=\zeromap^{h-n+1}(\trm(R_{\leq n-1}))=\clp^{n-2}(\trm(R))\text{,}$$
as required.
\end{proof}

The one-row clip erases the last row of $R$ and then rewrites the remaining rows by $\zeromap$, so a priori $\clp^p(R)$ depends on all of $R_{\leq n-1}$, not only on $R_{\leq p}$. In fact the intermediate rows are irrelevant. To state this, define for a bounded RC graph $X$ with $\hr(X)\geq p$
$$D^p(X)=\zeromap^{\,h_X-p}(X_{\leq p}),\qquad h_X=\max\bigl(p,\maxd(\wof{X_{\leq p}})\bigr)\text{,}$$
where $X_{\leq p}$ is regarded as a bounded RC graph of height $h_X$. By the weight-preservation argument following Definition~\ref{definition:cliptrim}, $D^p(X)$ is a bounded RC graph of height $p$ with $\wtt(D^p X)=\wtt(X)_{\leq p}$; moreover $D^p(X)=D^p(X_{\leq k})$ for every $k\geq p$, and $D^p(X)=\zeromap^{\,N-p}(X_{\leq p})$ for every $N\geq h_X$, since $\zeromap$ deletes the empty last row whenever the height exceeds $\maxd$.

\begin{lemma}\label{lemma:dtrim}
Let $X$ be a bounded RC graph with $\hr(X)\geq p\geq 1$. Then $\trm(D^p(X))=D^{p-1}(\trm X)$.
\end{lemma}
\begin{proof}
Let $h=h_X$. Iterating Lemma~\ref{lemma:ztrim} (the last row stays empty at each step by weight preservation),
$$\trm(D^p(X))=\trm\bigl(\zeromap^{h-p}(X_{\leq p})\bigr)=\zeromap^{h-p}\bigl(\trm(X_{\leq p})\bigr)\text{,}$$
and $\trm(X_{\leq p})=(\trm X)_{\leq p-1}$ regarded at height $h-1$, which is bounded by Lemma~\ref{L:trim}. Hence $h-1\geq h_{\trm X}$ and the right side is $D^{p-1}(\trm X)$.
\end{proof}

\begin{lemma}[Locality of $\clp^p$]\label{lemma:cliplocal}
Let $R$ be a bounded RC graph with $\hr(R)=n$ and let $0\leq p\leq n$. Then
$$\clp^p(R)=D^p(R)=\zeromap^{\,h_R-p}(R_{\leq p})\text{.}$$
In particular $\clp^p(R)$ depends only on the top $p$ rows of $R$.
\end{lemma}
\begin{proof}
We show $\clp^p=D^p$ on $\RC_n$ by induction on $n-p$, the case $n=p$ being trivial. For $n>p$, $\clp^p(R)=\clp^p(\clp^{n-1}R)=D^p(\clp^{n-1}R)$ by induction, and $\clp^{n-1}R=\zeromap^{h-n+1}(R_{\leq n-1})$ with $D^p(R)=D^p(R_{\leq n-1})$. It therefore suffices to prove
\begin{equation}\label{eq:clpzero}
D^p(\zeromap(Y))=D^p(Y)
\end{equation}
for every bounded RC graph $Y$ of height $m>p$ whose last row is empty. If $\maxd(\wof Y)<m$, then $\zeromap(Y)$ is $Y$ with its last row deleted, and \eqref{eq:clpzero} is clear. If $p\leq 1$, both sides of \eqref{eq:clpzero} are determined by their weight $(\wtt_1(Y),\ldots,\wtt_p(Y))$ (Lemmas~\ref{lemma:zeroweight} and~\ref{lemma:lastrow}).

Suppose $p\geq 2$ and $\maxd(\wof Y)=m$, and assume \eqref{eq:clpzero} for $p-1$. By Lemmas~\ref{lemma:dtrim} and~\ref{lemma:ztrim},
$$\trm\bigl(D^p(\zeromap Y)\bigr)=D^{p-1}(\trm\,\zeromap Y)=D^{p-1}(\zeromap\,\trm Y)=D^{p-1}(\trm Y)=\trm\bigl(D^p(Y)\bigr)\text{,}$$
where the third equality is \eqref{eq:clpzero} for $p-1$ applied to $\trm Y$ (or the trivial case if $\maxd(\wof{\trm Y})<m-1$). Thus rows $2,\ldots,p$ of the two sides of \eqref{eq:clpzero} coincide. A bounded RC graph $X$ is determined by $\trm X$ together with $\wof X$: the word of row $1$ is the unique strictly decreasing reduced word of $\wof X\cdot({\uparrow}\wof{\trm X})^{-1}$. Consequently \eqref{eq:clpzero} is equivalent to the equality of permutations
\begin{equation}\label{eq:clpperm}
\wof{D^p(\zeromap Y)}=\wof{D^p(Y)}\text{.}
\end{equation}

Two further reductions. First, rows $p+2,\ldots,m$ of $Y$ may be assumed empty: writing $D^{p}\circ D^{p+1}=D^{p}$ for the case $m=p+1$ at every level, \eqref{eq:clpzero} for general $m$ follows by induction on $m-p$, since $\zeromap Y$ has rows $p+1,\ldots,m-1$ empty when $Y$ has rows $p+1,\ldots,m$ empty. Second, $Y$ may be assumed to be a highest-weight element for the operators $e_1,\ldots,e_{p-1}$ of Definition~\ref{definition:crystal}. Indeed, let $1\leq i\leq p-1$ and suppose $e_i(Y)\neq\emptyset$. Then $e_i(Y)$ again has height $m$, empty last row and permutation $\wof Y$, so $\zeromap(e_iY)=e_i\zeromap(Y)$ by Corollary~\ref{corollary:crystaliso}. Moreover $D^p$ commutes with $e_i$ and $f_i$ on $\RC_{\leq p}$: erasing rows above $p$ and reassigning the height commute with $e_i$ trivially, and every $\zeromap$ in $D^p$ is applied at a height $\geq p+1>i$, so commutes by Corollary~\ref{corollary:crystaliso}. As $f_ie_i=\mathrm{id}$ wherever $e_i$ is defined,
$$D^p(\zeromap Y)=D^p\bigl(f_i\,\zeromap\,e_iY\bigr)=f_i\,D^p(\zeromap\,e_iY)\qquad\text{and}\qquad D^p(Y)=f_i\,D^p(e_iY)\text{,}$$
so \eqref{eq:clpzero} for $e_iY$ implies it for $Y$. Since $e_i(Y)=\emptyset$ automatically for $i\geq p$ (row $i+1$ is empty), the reduced case is that $Y_{\leq p}$ is a highest-weight RC graph.

The same commutation gives an invariant shared by the two sides of \eqref{eq:clpperm}. Let $C(Y)$ be the connected component of $Y$ under $e_1,\ldots,e_{p-1},f_1,\ldots,f_{p-1}$; all its elements have the same rows $p+1,\ldots,m$ as $Y$. By Propositions~\ref{proposition:zeroequal} and~\ref{proposition:zerocrystal}, $\zeromap$ restricts to a crystal isomorphism $C(Y)\to C(\zeromap Y)$; erasing rows above $p$ and reassigning the height is a crystal isomorphism $C(Y)\to C(Y_{\leq p})$; and each $\zeromap$-step inside $D^p$ is a crystal isomorphism onto its image. Hence
$$C\bigl(D^p(\zeromap Y)\bigr)\cong C(\zeromap Y)\cong C(Y)\cong C\bigl(D^p(Y)\bigr)$$
as $A_{p-1}$-crystals, and both $D^p(\zeromap Y)$ and $D^p(Y)$ are the highest-weight elements of their components. In particular the two components have the same Demazure character, so $D^p(\zeromap Y)$ and $D^p(Y)$ have the same weight and the same extremal weight (Definition~\ref{definition:crystal}), and by Theorem~\ref{theorem:keykey} the key polynomial $\kappa_{\mathrm{extwt}(D^pY)}$ occurs in both $\sch_{\wof{D^p(\zeromap Y)}}(x_1,\ldots,x_p)$ and $\sch_{\wof{D^pY}}(x_1,\ldots,x_p)$. This does not determine the permutation: distinct highest-weight elements of $\RC_p$ with the same weight and extremal weight exist, with distinct permutations.

\textbf{[Identity \eqref{eq:clpperm} for $\maxd(\wof Y)=m$, $p\geq 2$ remains to be written. It has been verified by computer for all bounded RC graphs of height at most $5$ and length at most $5$. Word-level form: after the two reductions above, $Y=T\cup\shup^p\mathtt{row}_{p+1}(c)$ is a highest-weight RC graph with word $\mathbf a\mathbf s$, $\mathbf a=\mathrm{word}(T)$, $\mathbf s=(p+c,\ldots,p+1)$. For a reduced word $\mathbf u$ and $k\geq 1$ let $L_{\leq k}(\mathbf u)$ denote the result of iterating the Little bump at the crossing of root $(m',m'+1)$, $m'=\maxd$ of the current permutation, until $\maxd\leq k$. By Proposition~\ref{proposition:zerocrystal}, $D^p(Y)=L_{\leq p}(\mathbf a)$, $\zeromap Y=L_{\leq m-1}(\mathbf a\mathbf s)=\mathbf a'\mathbf s'$ with $|\mathbf a'|=|\mathbf a|$, and $D^p(\zeromap Y)=L_{\leq p}(\mathbf a')$, so \eqref{eq:clpperm} reads
$$L_{\leq p}\bigl(\mathrm{pref}_{|\mathbf a|}\,L_{\leq m-1}(\mathbf a\mathbf s)\bigr)=L_{\leq p}(\mathbf a)\text{,}$$
required only for $\mathbf a$ the word of a highest-weight RC graph in $\RC_{\leq p}$ and $\mathbf s$ a single row of consecutive letters above $p$. The truncation at $\maxd\leq m-1$ before restarting on the prefix is essential: $\mathrm{pref}_{|\mathbf a|}L_{\leq p}(\mathbf a\mathbf s)\neq L_{\leq p}(\mathbf a)$ in general (already for $\mathbf a=(2)$, $\mathbf s=(3)$, $p=2$). By Proposition~\ref{proposition:zerocrystal}, $\zeromap$ is a composite of Little bumps at roots $(m',m'+1)$ with $m'>p$; each such bump, restricted to the top $p$ rows, is either the identity or a single Little bump of $Y_{\leq p}$ at a position whose root $(a,b)$ is a Bruhat cover of $\wof{Y_{\leq p}}$ with $a>p$ (verified for length $\leq 7$). Little bumps at covers $(a,b)$ with $a>p$ are well defined on $\RC_{\leq p}$: the position is nearly reduced, so the defect after each decrement is unique by \cite[Lemma~4]{little2003combinatorial}, and the resulting word is again a valid RC graph with the same row sequence, with no letter ever decremented from $1$ (verified for length $\leq 7$). Hence \eqref{eq:clpzero} is equivalent to the statement that $D^p(T)$ is unchanged by any such bump of $T\in\RC_{\leq p}$, i.e.\ to the confluence of the rewriting system on $\RC_{\leq p}$ generated by these bumps; the system terminates since each elementary step decreases $\Phi(T)=\sum_{(i,j)\in T}j$ by one, so by Newman's lemma local confluence suffices. Local form: write $L$ for the bump at the last-descent root $(\maxd(\wof T),\maxd(\wof T)+1)$, so $D^p(T)=L^{k}(T)$ for the first $k$ with $\maxd\leq p$. By induction on $\Phi$ it suffices to show that for every other $p$-high cover bump $\beta$ of $T$ some $L^{k}(\beta T)$, $k\geq 0$, is reachable from $L(T)$ by $p$-high cover bumps; then $D^p(\beta T)=D^p(L^k\beta T)=D^p(L T)=D^p(T)$ by the induction hypothesis applied to $L(T)$. In all cases with length $\leq 7$ this holds with $k\leq 3$ and at most two bumps from $L(T)$, in one of the shapes $L(\beta T)=L(T)$; $L(\beta T)=\beta'(LT)$ (the generic case, where $\beta'$ is the bump at the crossing of $LT$ corresponding to that of $\beta$, either by word position or by the pair of pipes it crosses); $\beta T=\beta'(LT)$ (when $\beta$ is at a root $(m,b)$ sharing the position $m$ with $L$); $L(\beta T)=\beta'\beta''(LT)$; or $L^2(\beta T)$, $L^3(\beta T)$ in place of $L(\beta T)$. A proof of the local statement by case analysis on the interaction of the two bump chains is what remains. Highest-weight elements are preserved by these bumps (verified for length $\leq 7$), and a highest-weight $T\in\RC_{\leq p}$ is the RC graph whose row $i$ is the $i$th row of the Edelman--Greene insertion tableau of its reading word taken from the last row upward \cite{morse2016crystal} (verified for length $\leq 7$). The reduced problem is therefore a rewriting system on Edelman--Greene tableaux with at most $p$ rows, each bump subtracting $1$ from a set of cells, with normal forms the tableaux whose permutation has last descent at most $p$; $D^p$ is a shape-preserving map between such tableaux. Negative observations: the one-step statement ``erasing rows $p+1,\ldots,m$ commutes with $\zeromap$'' is false; two bumps applied to the same $T$ need not commute even when they modify disjoint positions; $\wof{D^p(T)}$ is not determined by $\wof{T}$, $\wtt(T)$ and $\wof{\trm D^p(T)}$ (for $p=2$ the graphs $\{(1,2),(2,3)\}$ and $\{(1,4),(2,1)\}$ have the same permutation $13254$, weight and trimmed clip, but clips with permutations $231$ and $1423$), so the first row of $D^p(T)$ cannot be recovered from permutation-level data alone; and for highest-weight $T$ it is not determined by $\wtt(T)$, $\mathrm{extwt}(T)$, the pattern of $\wof{T}$ on positions $1,\ldots,p$, or the cells of the tableau with entries $\leq p$. Hamaker--Young's theorem gives that the Edelman--Greene recording tableau of the word of $T$ is a class invariant (trivial on highest weights, where it is the row superstandard tableau of the shape); a second invariant determining the first row of the normal form is what is missing.]}
\end{proof}
...
Thus $\wof{T}=u_{v'}:=w\,(\shup^pv')^{-1}$ where $v'=\wof{V'}$, and $\ell(u_{v'})=\ell(w)-\ell(v')$. Conversely, for any $v'$ with $\maxd(v')\leq q$ and $\ell(w(\shup^pv')^{-1})=\ell(w)-\ell(v')$, any $V'\in\RC_q(v')$, and any $T\in\RC_{\leq p}(u_{v'})$, the union $T\cup\shup^pV'$ is a reduced RC graph of $w$ with $p+q$ rows, hence lies in $\RC_{p+q}(w)$. Finally, by Lemma~\ref{lemma:cliplocal}, $\clp^p(T\cup\shup^pV')=\zeromap^{h-p}(T)$ with $h=\max(p,\maxd(u_{v'}))$.
...
Restricting to $\mathcal{R}$ gives a graded ring map $\alpha|_{\mathcal{R}}:\mathcal{R}\to\dcoma$.

Since $\alpha(\mathtt{row}(i))=\dsch^1_{s_i\cdots s_1}=[i]$ for all $i\ge 0$, the image of $\alpha|_{\mathcal{R}}$ contains the free generators $[i]$ of $\dcoma$, hence $\alpha|_{\mathcal{R}}$ is surjective. Conversely, by freeness of $\dcoma$, there is a unique graded ring homomorphism
$$
\iota:\dcoma\to\mathcal{R},\qquad \iota([i])=\mathtt{row}(i).
$$
Then $(\alpha|_{\mathcal{R}})\circ\iota=\mathrm{id}_{\dcoma}$, so $\alpha|_{\mathcal{R}}$ is also injective. Therefore $\alpha:\mathcal{R}\xrightarrow{\sim}\dcoma$ is an isomorphism of graded rings.

Finally, the family $\{\dsch_w^n\}_{n\ge 0,\, n\ge\maxd(w)}$ is a $\mathbb{Z}$-basis of $\dcoma$ by construction, and $\dsch_w^n(\mathtt{row})=\iota(\dsch_w^n)$, so
$$
\alpha\bigl(\dsch_w^n(\mathtt{row})\bigr)=\dsch_w^n.
$$
Applying the isomorphism $\iota$ shows that $\{\dsch_w^n(\mathtt{row})\}$ is a $\mathbb{Z}$-basis of $\mathcal{R}$.
\end{proof}
...
\end{theorem}
\begin{proof}
By Lemma~\ref{lemma:schubertrow} (whose first paragraph is the multiplicativity of $\alpha$), $\alpha$ is a ring homomorphism; it is surjective since every $\dsch_w^n$ is $\alpha(R)$ for any $R\in\RC_n(w)$, and $\RC_n(w)\neq\emptyset$ when $\maxd(w)\leq n$.
...
\emph{Right ideal.} Since $R_1$ and $R_2$ lie in the same connected component, it suffices to treat $R_2=e_i(R_1)$ for some $1\leq i\leq n-1$ (the case $R_2=f_i(R_1)$ is symmetric). For $R'$ of height $n+m$ with $\trm^n(R')=R$, the operator $e_i$ ($i\leq n-1$) acts on rows $i,i+1\leq n$, so $\trm^n(e_iR')=R$ when $e_iR'$ is defined, and $\clp^n(e_iR')=e_i(\clp^nR')$ by Corollary~\ref{corollary:crystaliso} applied to each $\zeromap$ step of each one-row clip in Definition~\ref{definition:cliptrim} (erasing a last row of index $>n$ commutes with $e_i$, and $\zeromap$ is a crystal isomorphism onto its image by Propositions~\ref{proposition:zeroequal} and~\ref{proposition:zerocrystal}, so $e_i$ is defined on $\clp^nR'$ if and only if it is defined on $R'$). Hence $e_i$ restricts to a bijection from the terms of $R_1\sqcupplus R$ onto the terms of $e_i(R_1)\sqcupplus R$, and corresponding terms $R'$, $e_i(R')$ have the same highest weight. Thus $R_1\sqcupplus R-R_2\sqcupplus R\in\Theta$.
\end{proof}
...
Both graphs have permutation $w_c=(1,2,3,5,7,4,10,6,8,9)$. They differ only in their first two rows: $R_1$ has row data $[(6,4),(9,),()]$ in the top three rows, while $R_2$ has $[(9,4),(6,),()]$. Applying $\clp^3$ (four one-row clips, which here amount to retaining the top $3$ rows and applying $\zeromap$ until the height is $3$; see Lemma~\ref{lemma:cliplocal}) to each yields the same $3$-row graph with extremal weight $a=(0,1,2)$:
$$\clp^3(R_1)=\clp^3(R_2)=
...
\item[$\clp^p(R)$] ``Clip'': the RC graph in $\RC_p$ obtained from $R$ by $\hr(R)-p$ one-row clips (erase the last row, then apply $\zeromap$ until the height drops by one); depends only on the top $p$ rows of $R$ (Definition~\ref{definition:cliptrim}, Lemma~\ref{lemma:cliplocal}).
