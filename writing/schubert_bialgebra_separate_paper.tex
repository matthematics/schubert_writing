...
\end{lemma}
\begin{proof}
	This is where the condition that $x_a^px_b^q=0$ unless $p=q$ when both $p,q>0$ comes in. It ensures that only monomials of the same length have nonzero products and preserves the structure of the coproduct as a homomorphism of rings.
\end{proof}
...
\begin{proof}
  ...
  Recall from Definition~\ref{definition:divdiff} that $\delta^v$, acting on the $y$ variables, satisfies $\delta^v[\sch_w(x;-y)]=\sch_{vw}(x;-y)$ whenever $\ell(vw)=\ell(w)-\ell(v)$. Thus,
