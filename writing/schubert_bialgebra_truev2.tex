...
\[
P_1(w)=\{v : w\dto{1}v\},\qquad
T_n(w)=\{w' : w\dto{n}w',\ \ell(w')=\ell(w)\}.
\]
\end{definition}
...
Fix $M$ with $w',w''\in S_M$ (the relation $\dto{1}$ does not depend on this choice).
Suppose $y\in S_M$ and $y\dto{1}u'$, and put $Y=\varphi_{1,M}(u')$. By Definitions 2.1.1 and 2.1.3 there are
...
\item Let $u'=w_{\trim S}$. By Lemma 4.3.4, $v\dto{n-1}u'$ and $v'\dto{n-1}u'$; by Lemma 4.3.3, $|Z(T)|=|T|$ and $|Z(T')|=|T'|$,
